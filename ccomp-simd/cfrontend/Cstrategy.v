...
(* Preciso extender isto para EvalStruct!!!
 - ???adicionar a relação eval_simple_rvaluel : expr -> list val -> Prop ???
*)
...
Proof.
  ...
  exists v; auto.
  intros.
  exists vargs; exists t; exists vres; exists m'; auto.
admit.
Qed.
...
(* receptiveness *)
  ...
  assert (HH:exists v, vres2 = v::nil).
  { destruct vres2. try inv EC2b. destruct vres2; try inv EC2b.
    exists v; reflexivity. }
  destruct HH as [v Hv]; subst.
...
Lemma eval_expression_to_steps:
   ...
   forall f k,
   star step ge (ExprState f a k e m) t (ExprState f (Eval v (typeof a)) k e m').
Admitted (*Proof (proj1 bigstep_to_steps).*).
...
Lemma eval_expr_to_steps:
   ...
   simple a' = true /\ typeof a' = typeof a /\
   star step ge (ExprState f (C a) k e m) t (ExprState f (C a') k e m').
Admitted (*Proof (proj1 (proj2 bigstep_to_steps)).*).
...
Lemma eval_exprlist_to_steps:
   ...
   star step ge (ExprState f (C (Ecall a1 (exprlist_app al2 al) ty)) k e m)
              t (ExprState f (C (Ecall a1 (exprlist_app al2 al') ty)) k e m').
Admitted (*Proof (proj1 (proj2 (proj2 bigstep_to_steps))).*).
...
Lemma exec_stmt_to_steps:
   ...
   exists S,
   star step ge (State f s k e m) t S /\ outcome_state_match e m' f k out S.
Admitted (*Proof (proj1 (proj2 (proj2 (proj2 bigstep_to_steps)))).*).
