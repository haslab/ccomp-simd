...
Fixpoint gentemps (tys: typelist): mon (list ident*list expr) :=
 match tys with
 | Tnil => ret (nil,nil)
 | Tcons t ts => do i <- gensym t;
                 do (ii,tt) <- gentemps ts;
                 ret (i::ii, Etempvar i t :: tt)
 end.
...
Definition make_set (id: ident) (l: expr) : statement :=
  ...
  | Some chunk =>
      ...
      Sbuiltin (id::nil) (EF_vload chunk) (Tcons typtr Tnil) ((Eaddrof l typtr):: nil)
  end.
...
Definition make_assign (l r: expr) : statement :=
  ...
  | Some chunk =>
      ...
      Sbuiltin nil (EF_vstore chunk) (Tcons typtr (Tcons ty Tnil))
...
Definition homogeneous_aggregate (ty:type) :=
  let n := 4%nat in (*FIXME: add runtime param*)
  match n with
  | O => false
  | S n => match ty with
           | Tstruct _ (Fcons _ t l) _ => andb (is_float_or_vec t)
...
Definition res_typelist (ty: type) :=
  match ty with
  | Tvoid => nil
  | Tstruct _ fld _ => if homogeneous_aggregate ty
                       then fieldlist_types fld
                       else ty::nil
  | _ => ty::nil
  end.
*)
...
(*
Fixpoint get_field {A} id ty (fl:fieldlist) (l:list A) {struct fl}: option A :=
 match fl, l with
 | Fcons id' ty' fl', v::l' => if peq id id'
                               then if type_eq ty ty' then Some v else None
                               else get_field id ty fl' l'
 | _, _ => None
 end.
*)

Fixpoint assign_aggregate_listval (a:expr) (fld:fieldlist) (tmps: list ident)
: mon (list statement):=
  match fld, tmps with
  | Fnil, _ => ret nil
  | Fcons id ty fld', v::vs => do s <- assign_aggregate_listval a fld' vs;
                               ret (Sassign (Efield a id ty) (Etempvar v ty)::s)
  | _, _ => error (msg "SimplExpr.transl_expr: assign_aggregate_listval")
  end.

Fixpoint assign_aggregate_fieldlist (a:expr) (fld:fieldlist) e
: mon (list statement):=
  match fld with
  | Fnil => ret nil
  | Fcons id ty fld' => do s <- assign_aggregate_fieldlist a fld' e;
                       ret (Sassign (Efield a id ty) (Efield e id ty)::s)
  end.

(*
Definition make_assign_aggregate (a:expr) (ty:type) (e:expr) : mon (list statement) :=
 match ty, e with
 | Tstruct _ fld _, Evalstruct tmps ty' =>
     if type_eq ty ty'
     then assign_aggregate_listval a fld tmps
     else error (msg "SimplExpr.transl_expr: assign_aggregate (wrong type)")
 | Tstruct _ fld _, _ =>
     if type_eq ty (typeof e)
     then assign_aggregate_fieldlist a fld e
     else error (msg "SimplExpr.transl_expr: assign_aggregate (wrong type)")
 | _, _ => error (msg "SimplExpr.transl_expr: assign_aggregate")
 end.
*)

Fixpoint assign_list lhs rhs :=
 match lhs, rhs with
 | nil, nil => ret nil
 | x::xs, y::ys => do rest <- assign_list xs ys;
                   ret ((Sassign x y)::rest)
 | _, _ => error (msg "SimplExpr.assign_list: mismatch on list sizes")
 end.
...
(*
Definition make_assign_aggregate dst (a:expr) (ty:type) (e:expr)
: mon (list statement) :=
 match e with
 | Csyntax.Ecall r1 rl2 ty =>
    do (sl1, a1) <- transl_expr For_val r1;
    do (sl2, al2) <- transl_exprlist rl2;
    do ts <- gensyms (rettype_of_type ty);
    do assign <- assign_ha (ha_selectors ty a) ts;
    ret (finish dst (sl1 ++ sl2 ++ Scall ts a1 al2 :: assign) a)
  | _ => error "XXX"
 end.
  | Csyntax.Ebuiltin ef tyargs rl ty =>
      ...
      match dst with
      | For_val | For_set _ =>
          do ts <- gensyms (res_typelist ty);
          ret (finish dst (sl ++ Sbuiltin ts ef tyargs al :: nil)
                          (if homogeneous_aggregate ty
                           then Evalstruct ts ty
                           else  match ts with
                           | t::nil => Etempvar t ty
                           | _ => dummy_expr
                           end))
      | For_effects =>
          ret (sl ++ Sbuiltin nil ef tyargs al :: nil, dummy_expr)
      end
...
Fixpoint transl_expr (dst: destination) (a: Csyntax.expr) : mon (list statement * expr) :=
  ...
  | Csyntax.Eval (Vvec tt n) ty =>
      ret (finish dst nil (Econst_vec n ty))
  | Csyntax.Eval _ ty =>
      error (msg "SimplExpr.transl_expr: Eval")
  | Csyntax.EvalStruct _ ty =>
      error (msg "SimplExpr.transl_expr: EvalStruct")
  ...
  | Csyntax.Ecast r1 ty =>
      do (sl1, a1) <- transl_expr For_val r1;
      ret (finish dst sl1 (if type_eq ty (Csyntax.typeof r1) then a1 else Ecast a1 ty))
  ...
  | Csyntax.Eassign l1 r2 ty =>
      ...
      then
        ...
        | Csyntax.Ecall r1 rl2 ty =>
          do (sl1', a1') <- transl_expr For_val r1;
          do (sl2', al2') <- transl_exprlist rl2;
          do (ids,ts) <- gentemps (rettypelist_of_type ty);
          do assign <- assign_list (ha_selectors ty a1) ts;
          ret (finish dst (sl1 ++sl1'++sl2' ++ Scall ids a1' al2'::assign) a1)
        | Csyntax.Ebuiltin ef tyargs rl ty =>
          do (sl, al) <- transl_exprlist rl;
          do (ids,ts) <- gentemps (rettypelist_of_type ty);
          do assign <- assign_list (ha_selectors ty a1) ts;
          ret (finish dst (sl1 ++sl++Sbuiltin ids ef tyargs al::assign) a1)
        | _ => (* assignment to HA are decomposed in its elements *)
          do (sl2, a2) <- transl_expr For_val r2;
          match dst with
          | For_val | For_set _ =>
             do assign <-assign_list (ha_selectors ty a1) (ha_selectors ty a2);
             ret (finish dst (sl1 ++ sl2 ++ assign) a1)
          | For_effects =>
             do assign <-assign_list (ha_selectors ty a1) (ha_selectors ty a2);
             ret (finish dst (sl1 ++ sl2 ++ assign) dummy_expr)
          end
        end
      else
        do (sl2, a2) <- transl_expr For_val r2;
        ...
        | For_val | For_set _ =>
             do t <- gensym ty2;
             ret (finish dst (sl1 ++ sl2
                  ++ Sset t a2 :: make_assign a1 (Etempvar t ty2) :: nil)
                         (Ecast (Etempvar t ty2) ty1))
         | For_effects =>
             ret (sl1 ++ sl2 ++ make_assign a1 a2 :: nil, dummy_expr)
        end
  ...
  | Csyntax.Ecall r1 rl2 ty =>
      ...
      | For_val | For_set _ =>
          if is_ha_type ty
          then error (msg "SimplExpr.transl_expr: HA function-call must be a rhs of an assignment")
          else
            do t <- gensym ty;
            ret (finish dst (sl1 ++ sl2 ++ Scall (t::nil) a1 al2 :: nil)
                        (Etempvar t ty))
      | For_effects =>
          ret (sl1 ++ sl2 ++ Scall nil a1 al2 :: nil, dummy_expr)
      end
  | Csyntax.Ebuiltin ef tyargs rl ty =>
      do (sl, al) <- transl_exprlist rl;
      match dst with
      | For_val | For_set _ =>
          if is_ha_type ty
          then error (msg "SimplExpr.transl_expr: HA builtinn-call must be a rhs of an assignment")
          else
            do t <- gensym ty;
            ret (finish dst (sl ++ Sbuiltin (t::nil) ef tyargs al :: nil)
                        (Etempvar t ty))
      | For_effects =>
          ret (sl ++ Sbuiltin nil ef tyargs al :: nil, dummy_expr)
      end
...
Fixpoint transl_stmt (s: Csyntax.statement) : mon statement :=
  ...
  | Csyntax.Sreturn (Some e) =>
      do (s', a) <- transl_expression e;
      ret (Ssequence s' (Sreturn (ha_selectors (typeof a) a)))
