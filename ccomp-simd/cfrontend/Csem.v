...
(** Homogeneous-aggregates are also dereferenced as pointers. They are
  promoted to [EvalStruct] only when used in [Efield] and [Eassign]
  expressions. This way they are properly handled when used in [lvalue]s. *)
...
Fixpoint loadv_iter chunk m b ofs n :=
 ...
 | S n' =>
   ...
   | Some v => match loadv_iter chunk m b 
                                (Int.add ofs (Int.repr (size_chunk chunk)))
                                n' with
               | Some vl => Some (v::vl)
               | _ => None
               end
...
Definition loadv_ha ty b ofs m : option (list val) :=
...
Inductive assign_loc {F V: Type} (ge: Genv.t F V) (ty: type) (m: mem) (b: block) (ofs: int): val -> trace -> mem -> Prop :=
...
Inductive cast_arguments: exprlist -> typelist -> list val -> Prop :=
  ...
  | cast_args_cons: forall v ty el targ1 targs v1 vl,
      ha_type ty = None -> (* HA should not appear in args (removed during parsing) *)
      sem_cast v ty targ1 = Some v1 -> cast_arguments el targs vl ->
      cast_arguments (Econs (Eval v ty) el) (Tcons targ1 targs) (v1 :: vl).
(*
  | cast_args_cons_ha_prt: forall ty ty' fld el targ1 targs v1 vl b ofs,
      ha_type ty = Some (size,ty') ->
      ty = targ1 -> cast_arguments m el targs vl ->
      loadv_fieldlist fld b ofs m = Some v1 ->
      cast_arguments m (Econs (Eval (Vptr b ofs) ty) el)
                     (Tcons targ1 targs) (v1 ++ vl)
  | cast_args_cons_ha_valStruct: forall ty ty' fld el targ1 targs v1 vl,
      homogeneous_aggregate ty = Some (ty',fld) ->
      ty = targ1 -> cast_arguments m el targs vl ->
      cast_arguments m (Econs (EvalStruct v1 ty) el) (Tcons targ1 targs)
                     (v1 ++ vl).
*)
...
(** A note on the handling of Homogeneous-Aggregates:
  HA can only appear in very specific places in C programs (handled in
  [StructReturn.ml]). Specifically, it only appears on the right-hand-side
  of an isolated assignment (an assignment in a [Sdo] statement).
  Accordingly, the semantics only assigns meaning to those constructions,
  leaving undefined other uses allowed by the syntax (e.g. directly projecting
  the outcome of a ha-returning function/builtin).                           *)
...
Inductive rred: expr -> mem -> trace -> expr -> mem -> Prop :=
  ...
  | red_assign: forall b ofs ty1 v2 ty2 m v t m',
      ...
      rred (Eassign (Eloc b ofs ty1) (Eval v2 ty2) ty1) m
         t (Eval v ty1) m'
  | red_assignHA: forall b ofs ty1 ty2 v2 m m',
      ty1 = ty2 -> 
      assign_loc_ha ty1 m b ofs v2 m' ->
      rred (Eassign (Eloc b ofs ty1) (EvalStruct v2 ty2) ty1) m
         E0 (Eval (Vptr b ofs) ty2) m'
...
(*
  | red_fieldStruct: forall m f tystruct lval v ty,
      get_field_homfstruct tystruct f lval = Some (ty,v) ->
      rred (Efield (EvalStruct lval tystruct) f ty) m
         E0 (Eval v ty) m
*)
  ...
  | red_builtin: forall ef tyargs el ty m vargs t vres m',
      ...
      external_call ef ge vargs m t (vres::nil) m' ->
      rred (Ebuiltin ef tyargs el ty) m
         t (Eval vres ty) m'
  | red_builtinHA: forall ef tyargs el ty hainfo m vargs t vres m',
      ha_type ty = Some hainfo ->
      cast_arguments el tyargs vargs ->
      external_call ef ge vargs m t vres m' ->
      rred (Ebuiltin ef tyargs el ty) m
         t (EvalStruct vres ty) m'.
...
Inductive sstep: state -> trace -> state -> Prop :=
  ...
  | step_return_2:  forall f v1 ty k e m v2 m',
      ...
      sstep (ExprState f (Eval v1 ty) (Kreturn k) e m)
         E0 (Returnstate (v2::nil) (call_cont k) m')
  | step_return_3:  forall f v1 ty ha k e m m',
      ha_type ty = Some ha ->
      ty = f.(fn_return) ->
      Mem.free_list m (blocks_of_env e) = Some m' ->
      sstep (ExprState f (EvalStruct v1 ty) (Kreturn k) e m)
         E0 (Returnstate v1 (call_cont k) m')
  | step_return_4:  forall f v1 b ofs ty k e m m',
      loadv_ha ty b ofs m = Some v1 ->
      ty = f.(fn_return) ->
      Mem.free_list m (blocks_of_env e) = Some m' ->
      sstep (ExprState f (Eval (Vptr b ofs) ty) (Kreturn k) e m)
         E0 (Returnstate v1 (call_cont k) m')
  ...
  | step_returnstate_0: forall v f e C ty k m,
      ha_type ty = None ->
      sstep (Returnstate (v::nil) (Kcall f e C ty k) m)
         E0 (ExprState f (C (Eval v ty)) k e m)
  | step_returnstate_1: forall v f e C ty ha k m,
      ha_type ty = Some ha ->
      sstep (Returnstate v (Kcall f e C ty k) m)
         E0 (ExprState f (C (EvalStruct v ty)) k e m).
