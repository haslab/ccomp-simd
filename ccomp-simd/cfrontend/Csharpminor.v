...
(*  | Ovecconst: int128 -> constant*).      (**r vec constant *)
...
Inductive cont: Type :=
  ...
  | Kcall: list ident -> function -> env -> temp_env -> cont -> cont.
                                        (**r return to caller *)
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | step_builtin: forall f optid ef bl k e le m vargs t vres m',
      ...
      step (State f (Sbuiltin optid ef bl) k e le m)
         t (State f Sskip k e (Cminor.set_ret optid vres le) m')
  ...
  | step_return_1: forall f a k e le m v m',
      eval_exprlist e le m a v ->
      Mem.free_list m (blocks_of_env e) = Some m' ->
      step (State f (Sreturn a) k e le m)
  ...
  | step_return: forall v optid f e le k m,
      step (Returnstate v (Kcall optid f e le k) m)
        E0 (State f Sskip k e (Cminor.set_ret optid v le) m).
