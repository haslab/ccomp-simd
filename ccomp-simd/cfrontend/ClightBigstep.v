...
Inductive outcome: Type :=
   ...
   | Out_return: (list val * typelist) -> outcome. (**r terminated by [return] *)
...
Definition outcome_result_value (out: outcome) (t: type) (v: list val) : Prop :=
  match out, t with
  | Out_normal, Tvoid => v = nil
(*  | Out_return None, Tvoid => v = Vundef*)
  | Out_return (v'::nil,Tcons t' Tnil), ty => ty <> Tvoid /\ forall x, sem_cast v' t' t = Some x /\ v = x::nil
  | Out_return (v',t'), ty => ty <> Tvoid /\ v = v'
  | _, _ => False
  end. 
...
Inductive exec_stmt: env -> temp_env -> mem -> statement -> trace -> temp_env -> mem -> outcome -> Prop :=
  ...
  | exec_Scall:   forall e le m optid a al tyargs tyres cconv vf vargs f t m' vres,
      ...
      exec_stmt e le m (Scall optid a al) (** FIX THIS!!! *)
                t (set_listtemp optid vres le) m' Out_normal
...
*)
  | exec_Sreturn_some: forall e le m al tl vl,
      eval_exprlist ge e le m al tl vl ->
      exec_stmt e le m (Sreturn al)
                E0 le m (Out_return (vl, tl))
...
with eval_funcall: mem -> fundef -> list val -> trace -> mem -> list val -> Prop :=
...
Inductive bigstep_program_terminates (p: program): trace -> int -> Prop :=
  | bigstep_program_terminates_intro: forall b f m0 m1 t r,
      ...
      eval_funcall ge m0 f nil t m1 (Vint r::nil) ->
...
*)
  | osm_return_some: forall a tl v k',
      call_cont k' = call_cont k ->
      eval_exprlist ge e le m a tl v ->
      outcome_state_match e le m f k
        (Out_return (v,tl)) (State f (Sreturn a) k' e le m).
...
(* return *)
  econstructor; split. apply star_refl. constructor. auto. auto.
(*
(* return some *)
