...
Inductive expr : Type :=
  ...
  | Econst_vec: int128 -> type -> expr    (**r long integer literal *)
(*
  | Evalstruct: list ident -> type -> expr(**r pseudo-expr for valstruct *)
*)
...
Inductive statement : Type :=
  ...
  | Scall: list ident -> expr -> list expr -> statement (**r function call *)
  | Sbuiltin: list ident -> external_function -> typelist -> list expr -> statement (**r builtin invocation *)
  | Ssequence : statement -> statement -> statement  (**r sequence *)
  | Sifthenelse : expr  -> statement -> statement -> statement (**r conditional *)
  | Sloop: statement -> statement -> statement (**r infinite loop *)
  | Sbreak : statement                      (**r [break] statement *)
  | Scontinue : statement                   (**r [continue] statement *)
  | Sreturn : list expr -> statement      (**r [return] statement *)
...
Fixpoint var_maxident (vars: list(ident * type)) : ident :=
  fold_right (fun x r=>if plt (fst x) r then r else (fst x)) 1%positive vars.
...
(* [ha_selectors] returns the list of all (Clight) selectors of a 
  given (C) HA variable *)
Definition idx_expr e ty i : expr :=
 Ederef (Ebinop Oadd e (Econst_int i (Tint I32 Signed noattr))
                (Tpointer ty noattr)) ty.

Fixpoint iter_idxsel e ty i n :=
 match n with
 | O => nil
 | S n' => idx_expr e ty (Int.repr i) :: iter_idxsel e ty (i+1) n'
 end.

Fixpoint ha_selectors (ty:type) (e:expr): (list expr) :=
 match ty with
 | Tstruct _ fl _ => ha_selectors_fl fl e
 | Tarray ty' size _ => (iter_idxsel e ty' 0 (Zabs_nat size))
 | _ => e::nil
 end
with ha_selectors_fl (l:fieldlist) e :=
 match l with
 | Fnil => nil
 | Fcons f t fl =>
     ha_selectors t (Efield e f t) ++ ha_selectors_fl fl e
 end.

(*
Fixpoint loadv_fieldlist (fld:fieldlist) b ofs m : option (list val) :=
 match fld with
 | Fnil => Some nil
 | Fcons _ ty fld' =>
    ...
    | By_value chunk => 
        ...
        | Some v => 
            match loadv_fieldlist fld' b
                   (Int.add ofs 
                    (Int.repr (size_chunk chunk))) m
            with
            | Some vl => Some (v::vl)
            | _ => None
            end
...
Inductive assign_loc_ha (ty : type) (m : mem) (b : block) (ofs : int)
: list val -> mem -> Prop :=
  | assign_loc_struct : forall lval (bytes : list memval) (m' : mem),
      (alignof_blockcopy ty | Int.unsigned ofs) ->
      encode_vals_ha ty lval = Some bytes ->
      Mem.storebytes m b (Int.unsigned ofs) bytes = Some m' ->
      assign_loc_ha ty m b ofs lval m'.
...
(* OBS: a ideia de ter argumentos do tipo homfstruct é interessante, mas
 parece causar demasiadas dificuldades!!!. Em particular, não é claro como
 se podem promover esses parâmetros a temporários... Parece sensato impor
 (pelo menos ao nível do Clight, mas talvez até do C) que não existam
 parametros de tipos homfstruct, i.e. que tenham sido já expandidos.
 Mas para já mantemos a coisa assim...
 *)
...
(*
  | bind_parameters_app:
      forall m id ty params v1 vl b m1 m2,
      PTree.get id e = Some(b, ty) ->
      assign_loc_ha ty m b Int.zero v1 m1 ->
      bind_parameters e m1 params vl m2 ->
      bind_parameters e m ((id, ty) :: params) (v1++vl) m2.
*)
...
*)
Fixpoint set_listtemp (lid: list ident) (lv: list val) (le: temp_env) :=
  match lid with
  | nil => le
  | id::lid => match lv with
               | nil => PTree.set id Vundef (set_listtemp lid nil le)
               | v::lv => PTree.set id v (set_listtemp lid lv le)
               end
  end.
...
Inductive eval_expr: expr -> val -> Prop :=
  ...
  | eval_Econst_vec:   forall i ty,
      eval_expr (Econst_vec i ty) (Vvec tt i)
...
Inductive eval_exprlist_args: list expr -> typelist -> list val -> Prop :=
  | eval_Enil_args:
      eval_exprlist_args nil Tnil nil
  | eval_Econs_args:   forall a bl ty tyl v1 v2 vl,
      eval_expr a v1 ->
      sem_cast v1 (typeof a) ty = Some v2 ->
      eval_exprlist_args bl tyl vl ->
      eval_exprlist_args (a :: bl) (Tcons ty tyl) (v2 :: vl).
(*
  | eval_Econs_ha_prt: forall a ty' fld el targ1 targs v1 vl b ofs,
      homogeneous_aggregate (typeof a) = Some (ty' fld) ->
      typeof a = targ1 -> eval_exprlist_args el targs vl ->
      eval_expr a (Vptr b ofs) ->
      loadv_fieldlist fld b ofs m = Some v1 ->
      eval_exprlist_args (a::el)
...
Inductive cont: Type :=
  ...
  | Kcall: list ident ->                  (**r where to store result *)
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | step_call:   forall f optid a al k e le m tyargs tyres cconv vf vargs fd,
      ...
      eval_exprlist_args e le m al tyargs vargs ->
  ...
  | step_builtin:   forall f optid ef tyargs al k e le m vargs t vres m',
      ...
      step (State f (Sbuiltin optid ef tyargs al) k e le m)
         t (State f Sskip k e (set_listtemp optid vres le) m')
  ...
  | step_return_0: forall f k e le m m',
      ...
      step (State f (Sreturn nil) k e le m)
        E0 (Returnstate nil (call_cont k) m')
  | step_return_1: forall f a k e le m v v' m',
      ha_type (typeof a) = None ->
      eval_expr e le m a v -> 
      sem_cast v (typeof a) f.(fn_return) = Some v' ->
      Mem.free_list m (blocks_of_env e) = Some m' ->
      step (State f (Sreturn (a::nil)) k e le m)
        E0 (Returnstate (v'::nil) (call_cont k) m')
  | step_return_2: forall f ha al k e le m vl m',
      ha_type f.(fn_return) = Some ha ->
      eval_exprlist e le m al (rettypelist_of_type f.(fn_return)) vl ->
      Mem.free_list m (blocks_of_env e) = Some m' ->
      step (State f (Sreturn al) k e le m)
        E0 (Returnstate vl (call_cont k) m')
  | step_skip_call: forall f k e le m m',
      is_call_cont k ->
      Mem.free_list m (blocks_of_env e) = Some m' ->
      step (State f Sskip k e le m)
        E0 (Returnstate nil k m')
  ...
  | step_returnstate: forall v optid f e le k m,
      step (Returnstate v (Kcall optid f e le k) m)
        E0 (State f Sskip k e (set_listtemp optid v le) m).
...
Inductive final_state: state -> int -> Prop :=
  | final_state_intro: forall r m,
      final_state (Returnstate (Vint r::nil) Kstop m) r.
...
(* receptiveness *)
  ...
  (* builtin *)
  exploit external_call_receptive; eauto. intros [vres2 [m2 [EC2a EC2b]]].
(*  destruct vres2; try inv EC2b.*)
  econstructor; econstructor; eauto.
  (* external *)
  exploit external_call_receptive; eauto. intros [vres2 [m2 [ECa EC2b]]]. 
(*  destruct vres2; try inv EC2b. destruct vres; try inv H4.*)
