...
let rec print_stmt p s =
  ...
  | Scall(ids, e1, el) ->
      fprintf p "@[<hv 2>%a =@ %a@,(@[<hov 0>%a@]);@]"
                ...
                print_expr e1
                print_expr_list (true, el)
  | Sbuiltin([], ef, tyargs, el) ->
      fprintf p "@[<hv 2>builtin %s@,(@[<hov 0>%a@]);@]"
                (name_of_external ef)
                print_expr_list (true, el)
  | Sbuiltin(ids, ef, tyargs, el) ->
 ...
 *)
  | Sreturn el ->
      fprintf p "return @[<hov 0>%a@];" print_expr_list (true,el)
  | Slabel(lbl, s1) ->
...
(*  | Sreturn None -> ()*)
  | Sreturn el -> collect_exprlist el
