...
(* volatile case *)
  intros. change (PTree.set id v le) with (set_listtemp (id::nil) (v::nil) le). econstructor.
...
Inductive match_cont : Csem.cont -> cont -> Prop :=
  ...
  | match_Kcall: forall f e C ty k optid tf le sl tk a dest tmps,
      tr_function f tf ->
      leftcontext RV RV C ->
      (forall v m, tr_top tge e (set_listtemp optid (v::nil) le) m dest (C (Csyntax.Eval v ty)) sl a tmps) ->
...
with match_cont_exp : destination -> expr -> Csem.cont -> cont -> Prop :=
  ...
  | match_Kreturn: forall k a tk,
      match_cont k tk ->
      match_cont_exp For_val a (Csem.Kreturn k) (Kseq (Sreturn (ha_selectors (typeof a) a)) tk).
...
(* break, continue, return 0 *)
  auto. auto. auto.
(* return 1 *)
admit (*  rewrite (tr_find_label_expression _ _ _ H0). auto.*).
(* switch *)
