...
Inductive tr_expr: temp_env -> destination -> Csyntax.expr -> list statement -> expr -> list ident -> Prop :=
  ...
  | tr_call_val: forall le dst e1 el2 ty sl1 a1 tmp1 sl2 al2 tmp2 t tmp,
      ...
      incl tmp1 tmp -> incl tmp2 tmp ->
      tr_expr le dst (Csyntax.Ecall e1 el2 ty) (** FIX THIS!!! *)
                   (sl1 ++ sl2 ++ Scall (t::nil) a1 al2 :: final dst (Etempvar t ty))
                   (Etempvar t ty) tmp
  | tr_builtin_effects: forall le ef tyargs el ty sl al tmp1 any tmp,
      ...
      tr_expr le For_effects (Csyntax.Ebuiltin ef tyargs el ty)
                   (sl ++ Sbuiltin nil ef tyargs al :: nil)
                   any tmp
(* FIX THIS!!!
...
(** FIX THIS!!! *)
Inductive tr_expression_list: Csyntax.expr -> statement -> list expr -> Prop :=.
...
Inductive tr_stmt: Csyntax.statement -> statement -> Prop :=
  ...
  | tr_return_none:
      tr_stmt (Csyntax.Sreturn None) (Sreturn nil)
  | tr_return_some: forall r s' al,
      tr_expression_list r s' al ->
      tr_stmt (Csyntax.Sreturn (Some r)) (Ssequence s' (Sreturn al))
...
(* val *)
  simpl in H. destruct v; try destruct t; monadInv H; exists (@nil ident); split; auto with gensym.
Opaque makeif.
...
destruct (type_eq ty (Csyntax.typeof r)); intros.
admit.
constructor; auto.
(* seqand *)
  monadInv H1. exploit H; eauto. intros [tmp1 [A B]].
