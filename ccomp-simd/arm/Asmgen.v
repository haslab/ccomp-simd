...
Definition vreg_of (r: mreg) : res vreg :=
  match preg_of r with VR mr => OK mr | _ => Error(msg "Asmgen.vreg_of") end.
...
Definition loadind (base: ireg) (ofs: int) (ty: typ) (dst: mreg) (k: code) :=
  ...
  | Tvec tt =>
      do r <- vreg_of dst;
      OK (indexed_memory_access (Pvldv r) mk_immed_mem_float base ofs k)
  end.
...
Definition storeind (src: mreg) (base: ireg) (ofs: int) (ty: typ) (k: code) :=
  ...
  | Tvec tt =>
      do r <- vreg_of src;
      OK (indexed_memory_access (Pvstv r) mk_immed_mem_float base ofs k)
  end.
...
Definition transl_memory_access_vec
     (mk_instr: vreg -> ireg -> int -> instruction)
     (mk_immed: int -> int)
     (dst: mreg) (addr: addressing) (args: list mreg) (k: code) :=
  do rd <- vreg_of dst;
  transl_memory_access
...
Definition transl_load (chunk: memory_chunk) (addr: addressing)
  ...
  | Mvec tt =>
      transl_memory_access_vec Pvldv mk_immed_mem_float dst addr args k
  end.
...
Definition transl_store (chunk: memory_chunk) (addr: addressing)
  ...
  | Mvec tt =>
      transl_memory_access_vec Pvstv mk_immed_mem_float src addr args k
  end.
