...
Inductive vreg: Type :=
  | VR0: vreg  | VR1: vreg  | VR2: vreg  | VR3: vreg
  | VR4: vreg  | VR5: vreg  | VR6: vreg  | VR7: vreg
  | VR8: vreg  | VR9: vreg  | VR10: vreg  | VR11: vreg
  | VR12: vreg  | VR13: vreg  | VR14: vreg  | VR15: vreg.
...
Lemma vreg_eq: forall (x y: vreg), {x=y} + {x<>y}.
Proof. decide equality. Defined.
...
Inductive preg: Type :=
  ...
  | VR: vreg -> preg                    (**r double-precision VFP float registers *)
  | CR: crbit -> preg                   (**r bits in the condition register *)
  | PC: preg.                           (**r program counter *)

Coercion IR: ireg >-> preg.
Coercion FR: freg >-> preg.
Coercion VR: vreg >-> preg.
Coercion CR: crbit >-> preg.

Lemma preg_eq: forall (x y: preg), {x=y} + {x<>y}.
Proof.
decide equality. apply ireg_eq. apply freg_eq. apply vreg_eq.
apply crbit_eq.
Defined.
...
Inductive instruction : Type :=
  ...
  (* Vector floating-point coprocessor instructions (VFP vector operations) *)
  | Pvmov: vreg -> vreg -> instruction              (**r vec move *)
  | Pvldv: vreg -> ireg -> int -> instruction       (**r (PSEUDO) vec128 load *)
  | Pvstv: vreg -> ireg -> int -> instruction       (**r (PSEUDO) vec128 store *)
...
Definition exec_instr (f: function) (i: instruction) (rs: regset) (m: mem) : outcome :=
  ...
  (* floating-point coprocessor vector instructions *)
  | Pvmov r1 r2 =>
      Next (nextinstr (rs#r1 <- (rs#r2))) m
  | Pvldv r1 r2 n =>
      exec_load (Mvec tt) (Val.add rs#r2 (Vint n)) r1 rs m
  | Pvstv r1 r2 n =>      
      exec_store (Mvec tt) (Val.add rs#r2 (Vint n)) r1 rs m
  (* Pseudo-instructions *)
  | Pallocframe sz pos =>             
...
Definition preg_of (r: mreg) : preg :=
  ...
  | Q0 => VR0 | Q1 => VR1 | Q2 => VR2 | Q3 => VR3
  | Q4 => VR4 | Q5 => VR5 | Q6 => VR6 | Q7 => VR7
  | Q8 => VR8 | Q9 => VR9 | Q10 => VR10 | Q11 => VR11
  | Q12 => VR12 | Q13 => VR13 | Q14 => VR14 | Q15 => VR15
  end.
...
Definition loc_external_result (sg: signature) : option (list preg) :=
  match loc_result sg with
  | None => None
  | Some lres => Some (map preg_of lres)
  end.
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_step_external:
      forall b ef args res rs m t rs' m' lres,
      rs PC = Vptr b Int.zero ->
      Genv.find_funct_ptr ge b = Some (External ef) ->
      external_call' ef ge args m t res m' ->
      extcall_arguments rs m (ef_sig ef) args ->
      loc_external_result (ef_sig ef) = Some lres ->
      rs' = (set_regs (lres) res rs)#PC <- (rs IR14) ->
      step (State rs m) t (State rs' m').
