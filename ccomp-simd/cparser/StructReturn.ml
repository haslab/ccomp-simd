...
type return_kind = 
  | Ret_homfstruct (**r a homogeneous float/vec aggregate *)
...
let classify_return env ty =
  if is_composite_type env ty then 
  begin
     if is_homfstruct_type env ty then
       Ret_homfstruct 
    else begin
...
let rec transf_type env t =
  ...
  | TFun(tres, None, vararg, attr) ->
      ...
      let tres'' =
        ...
        | Ret_scalar | Ret_homfstruct -> tres'
        | Ret_ref -> TVoid []
        | Ret_value ty -> ty in
      TFun(tres'', None, vararg, attr)
  | TFun(tres, Some args, vararg, attr) ->
      let args' = (*List.map (transf_funarg env)*) transf_funargs env vararg args in
      let tres' = transf_type env tres in
      begin match classify_return env tres with
      | Ret_scalar | Ret_homfstruct->
          TFun(tres', Some args', vararg, attr)
      | Ret_ref ->
...
and transf_funargs env vararg = function
  | [] -> []
  | (i,t)::xs -> 
     match vararg, homfstruct_check env t with
     | false, Some (ty,l) ->
	let ids = List.map (fun _->Env.fresh_ident ("_"^i.name),ty) l in
	ids @ transf_funargs env vararg xs
     | _,_ -> (i,transf_type env t):: transf_funargs env vararg xs
(* 
...
and transf_call env ctx opt_lhs fn args ty =
  let rec xpnd_args = function
      | [] -> []
      | x::xs ->
	 let e = transf_expr env Val x in
	 match homfstruct_check env e.etyp with
	 | Some (ty,l) -> List.map (fun f->f e) l @ xpnd_args xs
	 | _ -> e :: xpnd_args xs in
  let ty' = transf_type env ty in
  let fn' = transf_expr env Val fn in
  let args' = if is_vararg_function_type env fn'.etyp
	      then List.map (transf_expr env Val) args
	      else xpnd_args args in
  let opt_eassign e =
    match opt_lhs with
    | None -> e
...
let rec transf_stmt s =
  ...
  | Sreturn(Some e) ->
      ...
      | (Ret_scalar|Ret_homfstruct), None ->
...
let transf_fundef env f =
  reset_temps();
  let rec init_temps ident ty fl il k =
    match fl, il with
    | [], [] -> k
    | f::ff, (i,t)::ll ->
       sseq no_loc (sassign no_loc (f {edesc=EVar ident;etyp=ty})
			    {edesc=EVar i;etyp=t})
	    (init_temps ident ty ff ll k)
    | _, _ -> assert false in
  let rec xpnd_params = function
    | [] -> ([],[],fun x->x)
    | (i,t)::xs ->
       begin match homfstruct_check env t with
       | Some (ty,l) ->
	  let ids = List.map (fun _->Env.fresh_ident ("_"^i.name),ty) l in
	  let (a,b,c) = xpnd_params xs in
	  ( ids@a
	  , (Storage_default,i,t,None)::b
	  , fun x->(init_temps i t l ids (c x)))
       | _ -> let (a,b,c) = xpnd_params xs in
	      ((i,transf_type env t) :: a, b, c)
       end in
  let ret = transf_type env f.fd_ret in
  let (params,ldecl,pinit) =
    if f.fd_vararg
    then (List.map (fun (id, ty) -> (id, transf_type env ty)) f.fd_params
	 ,[],fun x->x)
    else xpnd_params f.fd_params in
  ...
  {f with fd_ret = ret1; fd_params = params1;
          fd_locals = ldecl @ f.fd_locals @ temps; fd_body = pinit body1}
