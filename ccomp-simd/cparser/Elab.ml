...
(* 6.5.2 Postfix expressions *)
  ...
  | INDEX(a1, a2) ->            (* e1[e2] *)
      ...
      let tres, bb =
        ...
        | (TPtr(t, _) | TArray(t, _, _)), ((TInt _ | TEnum _) as ti) -> 
	   if vector_size ti = 0 then t, b1
	   else error "subscripting index is not an integer" 
        | ((TInt _ | TEnum _) as ti), (TPtr(t, _) | TArray(t, _, _)) ->
	   if vector_size ti = 0 then t, b1
	   else error "subscripting index is not an integer" 
        | (TInt _ | TFloat _) as tb, ((TInt _ | TEnum _) as ti) -> 
	   if vector_size ti = 0
	   then if vector_size tb > 0 
		then if is_lvalue b1
		     then vector_basetype tb, elab (UNARY(ADDROF, a1))
		     else error "subscriting a non-lvalue vector"
		else error "subscripting in non-vector"
	   else error "subscripting index is not an integer" 
        | t1, t2 -> error "incorrect types for array subscripting" in
      { edesc = EBinop(Oindex, bb, b2, TPtr(tres, [])); etyp = tres }
...
let rec elab_init loc env ty ile =
  ...
  | (TInt _ | TFloat _ | TPtr _ | TEnum _) as tt ->
      begin match ile with
      (* scalar = elt *)
      | SINGLE_INIT a :: ile1 ->
      ...
      | COMPOUND_INIT ile1 :: ile2 ->
	 let rec elab_init_Intvec vs ik ile = 
	   let iksize = sizeof_ikind(ik) in
	   if vs mod iksize = 0
	   then if vs = 0
		then ([], ile)
		else match ile with
		     | ile1::ile2 ->
			let (i1, _) = elab_init loc env (TInt (ik, [])) [ile1] in
			let (i2, rest) = elab_init_Intvec (vs-iksize) ik ile2 in
			(i1::i2, rest)
		     | [] -> let (i2, _) = elab_init_Intvec (vs-iksize) ik ile2 in
			     ((Init_single (intconst 0L ik)) :: i2, [])
	   else fatal_error loc "invalid vector size in initializer for type@ %a"
			    Cprint.typ ty
	 and elab_init_Floatvec vs fk ile = 
	   let fksize = sizeof_fkind(fk) in
	   if vs mod fksize = 0
	   then if vs = 0
		then ([], ile)
		else match ile with
		     | ile1::ile2 ->
			let (i1, _) = elab_init loc env (TFloat (fk, [])) [ile1] in
			let (i2, rest) = elab_init_Floatvec (vs-fksize) fk ile2 in
			(i1::i2, rest)
		     | [] -> let (i2, _) = elab_init_Floatvec (vs-fksize) fk ile2 in
			     ((Init_single floatconst0) :: i2, [])
	   else fatal_error loc "invalid vector size in initializer for type@ %a"
			    Cprint.typ ty
	 in
	 match tt with 
	 | TInt (ik,a) when vectorsize_attribute a > 0 ->
	    let (il,rem) = elab_init_Intvec (vectorsize_attribute a) ik
					    (project_init loc ile1)
	    in if rem <> [] then
		 warning loc "excess elements at end of vector initializer";
               (Init_array il, ile2)
	 | TFloat (fk,a) when vectorsize_attribute a > 0 ->
	    let (il,rem) = elab_init_Floatvec (vectorsize_attribute a) fk
					      (project_init loc ile1)
	    in if rem <> [] then
		 warning loc "excess elements at end of vector initializer";
               (Init_array il, ile2)
	 | _ -> fatal_error loc "compound initializer for type@ %a" Cprint.typ ty
      end
  | _ ->
...
let elab_initial loc env ty ie =
  ...
  | ((TArray _ | TStruct _ | TUnion _), COMPOUND_INIT _) ->
      ...
      Some i
  (* vectype = { elt, ..., elt } *)
  | ((TInt _ | TFloat _), COMPOUND_INIT _) ->
      let (i, rem) = elab_init loc env ty [ie] in  
      if rem <> [] then
        warning loc "excess elements at end of vector data initializer";
      Some i
  | _, _ ->
