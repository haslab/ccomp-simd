...
let rec local_initializer loc env path init k =
  ...
  | Init_single e ->
      sdoseq loc
        ...
        k
  | Init_array il ->
      let ty = unroll env path.etyp in
      let ty_elt =
        match ty with
	...
	| TFloat (fk,a) when vectorsize_attribute a > 0 ->
      ...
      let rec vec_init pos = function
        | [] -> k
        | i :: il -> 
	   let pderef = { edesc = EUnop(Oaddrof, path)
			; etyp = TPtr(ty_elt, []) } in
            local_initializer loc env
              { edesc = EBinop(Oindex, pderef,
			       intconst pos IInt, TPtr(ty_elt, []));
                etyp = ty_elt }
              i
              (vec_init (Int64.succ pos) il) in
      (match ty with
       | TArray _ -> array_init 0L il
       | _ -> vec_init 0L il)
  | Init_struct(id, fil) ->
