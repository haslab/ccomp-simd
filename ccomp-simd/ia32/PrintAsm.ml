...
let t128_reg_name = function
  | XXMM0 -> "%xmm0"  | XXMM1 -> "%xmm1"  | XXMM2 -> "%xmm2"  | XXMM3 -> "%xmm3"
  | XXMM4 -> "%xmm4"  | XXMM5 -> "%xmm5"  | XXMM6 -> "%xmm6"  | XXMM7 -> "%xmm7"
...
let xreg oc r = output_string oc (t128_reg_name r)
...
let stack_alignment =
  match target with
  | ELF | Cygwin -> 16               (* minimum is 4, 8 is better for perfs *)
  | MacOS -> 16                     (* mandatory *)
...
(* places immediate arguments in appropriate positions *)
let rec mixargs immargs imms args =
  match immargs with
  | x::xs -> 
     if x==0
     then (match imms with
	   | i::is -> (sprintf "$0x%lx" (camlint_of_coqint i)) :: mixargs xs is args
	   | [] -> assert false (*invalid_arg "Fatal error: immediate arg. not found";[]*))
     else (match args with
           | a::aa -> (spreg a) :: mixargs ((x-1)::xs) imms aa
	   | [] -> assert false (*invalid_arg "Fatal error: not enough args.";[]*))
  | [] -> 
     match args with
     | a::aa -> (spreg a) :: mixargs [] imms aa
     | [] -> (match imms with
	      | _::_ -> assert false (*invalid_arg "Fatal error: not enough arguments";[]*)
	      | _ -> [])

(* print simd builtin *)
let is_formatted_asm s =
 try let _ = String.index s '%' in true
 with Not_found -> false

(** NOTE: begin new print formatted method **)
(*let rec print_formatted_asm oc s l =*)
(*  try *)
...
(*    fprintf oc "%s%s" pref (List.nth l x);*)
(*    print_formatted_asm oc suff l*)
(*  with Not_found -> fprintf oc "%s" s*)

let rec int8_reg_name_string x =
  match x with
  | "%eax" -> "%al"
  | "%ebx" -> "%bl"
  | "%ecx" -> "%cl"
...
let rec high_int8_reg_name_string x =
  match x with
  | "%eax" -> "%ah"
  | "%ebx" -> "%bh"
  | "%ecx" -> "%ch"
...
let rec select_string x s =
  match x with
  | 'l' -> int8_reg_name_string s
  | 'h' -> high_int8_reg_name_string s
  | _   -> s
...
let rec print_formatted_asm oc s l =
  try 
    ...
    let x = Char.code(s.[p+1]) - Char.code('0') in
    if (0 <= x && x <= 9)
    then let pref = String.sub s 0 p in
         let suff = String.sub s (p+2) (String.length s - (p+2)) in
         fprintf oc "%s%s" pref (List.nth l x);
         print_formatted_asm oc suff l

    else if (s.[p+1] = 'l' || s.[p+1] = 'h')
         then let pref = String.sub s 0 p in
              let x = Char.code(s.[p+2]) - Char.code('0') in
              let suff = String.sub s (p+3) (String.length s - (p+3)) in
              let reg = select_string s.[p+1] (List.nth l x) in
              fprintf oc "%s%s" pref reg;
              print_formatted_asm oc suff l
         else assert false;
  with Not_found -> fprintf oc "%s" s

(** NOTE: end new print formatted method **)

let print_simd_builtin oc x imms args res =
(*  match x.bname, imms, args, res with
  (* specially handled builtins *)
  | _ -> *)
    let largs = mixargs x.bimmargs imms args in
    let l = match res with
            | [r] -> spreg r :: largs
            | _ -> invalid_arg ("Fatal error: in builtin "^x.bname)
    in
    if is_formatted_asm x.basm
    then (fprintf oc "\t"; print_formatted_asm oc (String.lowercase x.basm) l)
    else (fprintf oc "\t%s " (String.lowercase x.basm);
          fprintf_sep oc "," (List.rev (if x.btwoaddr then largs else l)));
    fprintf oc "\n"

let print_builtin_inline oc id imms args res =
  ...
  | "__builtin_bswap16", [IR a1], [IR res] ->
      if a1 <> res then
        fprintf oc "	movl	%a, %a\n" ireg a1 ireg res;
      fprintf oc "	rolw	$8, %a\n" ireg16 res
  | "__builtin_bswap64", [IR a1; IR a2], [IR res1; IR res2] ->
      assert (a1=res2 && a2=res1);
      fprintf oc "	bswap	%a\n" ireg a1;
      fprintf oc "	bswap	%a\n" ireg a2;
  (* Float arithmetic *)
  ...
  (* Access to the RDTSC counter *)
  | "__builtin_rdtsc", [], [IR a; IR d] ->
      fprintf oc "	xor %a, %a\n" ireg a ireg a; (* a "is" %eax *)
      fprintf oc "	cpuid\n";
      fprintf oc "	rdtsc\n";
  (* Catch-all *)  
  | _ -> match simd_builtin_find id with
	 | Some blt -> 
	    if String.sub blt.basm 0 3 == "NYI"
	    then invalid_arg ("Sorry: not yet implemented builtin " ^ name)
	    else print_simd_builtin oc blt imms args res
...
let print_instruction oc = function
  ...
  | Pmov_xx(rd, r1) ->
      fprintf oc "	movdqa	%a, %a\n" xreg r1 xreg rd
  | Pmov_xm(rd, a) ->
      fprintf oc "	movdqa	%a, %a\n" addressing a xreg rd
  | Pmov_mx(a, r1) ->
      fprintf oc "	movdqa	%a, %a\n" xreg r1 addressing a
(** NOTE : begin : TODO : TESTME WITH ALIGNED MOVES **
  | Pmov_xx(rd, r1) ->
      fprintf oc "	movdqu	%a, %a\n" xreg r1 xreg rd
  | Pmov_xm(rd, a) ->
      fprintf oc "	movdqu	%a, %a\n" addressing a xreg rd
  | Pmov_mx(a, r1) ->
      fprintf oc "	movdqu	%a, %a\n" xreg r1 addressing a
** NOTE : end : TODO : TESTME WITH ALIGNED MOVES **)
  ...
  | Pbuiltin(ef, args, res) ->
      begin match ef with
      | EF_builtin(name, imms, sg) ->
          print_builtin_inline oc name imms args res
      | EF_vload chunk ->
