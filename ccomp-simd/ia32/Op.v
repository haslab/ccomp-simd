...
(*c Boolean tests: *)
  ...
  | OX: operationX -> operation		(** T128 ops. *)
  .
...
Proof.
  ...
  generalize eq_operationX; intro.
  decide equality.
  apply peq.
  apply eq_addressing.
  apply eq_condition.
...
Definition eval_operation
  ...
  | OX op, args => eval_operationX genv sp op args m
  | _, _ => None
  end.
...
Proof with (try exact I).
  ...
  eapply type_of_operationX_sound; eauto.
Qed.
...
Definition op_depends_on_memory (op: operation) : bool :=
  match op with
  | Ocmp (Ccompu _) => true
  | OX op => opX_depends_on_memory op 
  | _ => false
  end.
...
Proof.
  ...
  eapply opX_depends_on_memory_correct; eauto.
Qed.
...
Proof.
  ...
  simpl; constructor.
  eapply eval_operationX_inj; eauto.
Qed.
