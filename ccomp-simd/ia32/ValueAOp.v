...
Definition eval_static_operationX (op: operationX) (vl: list aval): aval :=
  match op with
  | _ => Ifptr Pbot
  end.
...
Definition eval_static_operation (op: operation) (vl: list aval): aval :=
  ...
  | OX op, vs => eval_static_operationX op vs
  | _, _ => Vbot
  end.
...
Theorem eval_static_operationX_sound:
  forall op vargs m vres aargs,
  eval_operationX ge (Vptr sp Int.zero) op vargs m = Some vres ->
  list_forall2 (vmatch bc) vargs aargs ->
  vmatch bc vres (eval_static_operationX op aargs).
Proof.
  unfold eval_operationX, eval_static_operationX; intros;
  destruct op; InvHyps; eauto with va;
  inv H1; inv H0; inv H; constructor.
(*  destruct a1; destruct a0; inv H; constructor.*)
Qed.
...
Proof.
  ...
  eapply eval_static_operationX_sound; eauto.
Qed.
