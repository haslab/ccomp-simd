...
Definition make_env (b: bounds) :=
  ...
  let ol := align (ofcs + 8 * b.(bound_float_callee_save)) 16 in (* locals *)
  let ostkdata := align (ol + 4 * b.(bound_local)) 16 in (* stack data *)
...
Proof.
  ...
  generalize (align_le (fe.(fe_ofs_float_callee_save) + 8 * b.(bound_float_callee_save)) 16 (refl_equal _)).
  generalize (align_le (fe.(fe_ofs_local) + 4 * b.(bound_local)) 16 (refl_equal _)).
...
(*  apply align_le; auto with zarith.*)
Qed.
