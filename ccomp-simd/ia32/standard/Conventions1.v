...
Definition sse_caller_save_regs := Y0 :: Y1 :: Y2 :: Y3 :: Y4 :: Y5 :: Y6 :: Y7 :: nil.
...
Definition destroyed_at_call :=
  FP0 :: int_caller_save_regs ++ float_caller_save_regs ++ sse_caller_save_regs.

Lemma mreg_seq_destroyed_at_call: forall r1 r2,
  mreg_seq r1 r2 -> In r2 destroyed_at_call -> In r1 destroyed_at_call.
Proof.
unfold mreg_seq, mreg_eqnorm, destroyed_at_call, int_caller_save_regs,
 float_caller_save_regs, sse_caller_save_regs; simpl app; intros.
destruct r1; destruct r2; try inv H; auto; compute; tauto.
Qed.
...
Lemma index_int_callee_save_seq: forall r1 r2,
 mreg_seq r1 r2 -> index_int_callee_save r1 = index_int_callee_save r2.
Proof.
unfold mreg_seq.
destruct r1; destruct r2; simpl; intro H; reflexivity || inv H.
Qed.

Lemma index_float_callee_save_seq: forall r1 r2,
 mreg_seq r1 r2 -> index_float_callee_save r1 = index_float_callee_save r2.
Proof.
unfold mreg_seq.
destruct r1; destruct r2; simpl; intro H; reflexivity || inv H.
Qed.
...
(* compcert-simd: simd-arguments are 16-byte aligned (4 int slots) *)
...
Fixpoint loc_arguments_rec
  ...
  | T128 :: tys => S Outgoing (align ofs 4) T128 :: loc_arguments_rec tys ((align ofs 4) + 4)
  end.
...
Fixpoint size_arguments_rec
  ...
  match tyl with
  | nil => ofs
  | T128 :: tys => size_arguments_rec tys ((align ofs 4) + typesize T128)
...
Definition loc_argument_acceptable (l: loc) : Prop :=
  ...
  | S Outgoing ofs ty => ofs >= 0 /\ ty <> Tlong /\ (ty<>T128 \/ (4|ofs))
  | _ => False
  end.
...
Remark loc_arguments_rec_charact:
  ...
  match l with
  | S Outgoing ofs' ty => ofs' >= ofs /\ ty <> Tlong /\ (ty<>T128 \/ (4|ofs'))
  | _ => False
  end.
Proof.
  ...
  destruct H.
  destruct a.
- destruct H. subst l. split. omega. split; try congruence. left; congruence.
  exploit IHtyl; eauto.
  destruct l; auto. destruct sl; auto. 
  intros [H1 [H2 [H3|H3]]]; intuition omega.
- destruct H. subst l. split. omega. split; try congruence. left; congruence.
  exploit IHtyl; eauto.
  destruct l; auto. destruct sl; auto. intuition omega.
- destruct H. subst l; split; [omega|]. split; [|left]; congruence.
  destruct H. subst l; split; [omega|]. split; [|left]; congruence.
  exploit IHtyl; eauto.
  destruct l; auto. destruct sl; auto. intuition omega.
- destruct H. subst l. split. generalize (align_le ofs 4). omega. 
   split; [congruence|]. right; eapply align_divides; auto with zarith.
  exploit IHtyl; eauto.
  destruct l; auto. destruct sl; auto. intros [H1 H2]. 
  generalize (align_le ofs 4). intuition omega.
- destruct H. subst l. split. omega. split; try congruence. left; congruence.
  exploit IHtyl; eauto.
  destruct l; auto. destruct sl; auto. intuition omega.
Qed.
...
Proof.
  ...
  destruct a; try apply IHtyl.
  simpl; apply Zle_trans with (align ofs0 4 + 4); auto. 
  generalize (align_le ofs0 4); omega.
Qed.
