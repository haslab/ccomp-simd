...
Definition index_valid (idx: frame_index) :=
  ...
  | FI_link => True
  | FI_retaddr => True
  | FI_local x ty => ty <> Tlong /\ 0 <= x /\ x + typesize ty <= b.(bound_local) /\ (ty<>T128 \/ (4|x))
  | FI_arg x ty => ty <> Tlong /\ 0 <= x /\ x + typesize ty <= b.(bound_outgoing) /\ (ty<>T128 \/ (4|x))
...
Lemma aligned_16_4x: forall x y,
  (16|x) -> (4|y) -> (16|x+4*y).
Proof.
intros. apply Z.divide_add_r; auto. inv H0.
exists x0; omega.
Qed.
...
Proof.
  ...
  assert (4 | fe_ofs_local fe).
   apply Zdivides_trans with 16; auto with align_4.
...
Proof.
  ...
  destruct ty; InvBooleans; try (
   split; [discriminate|split;[omega|split;[omega|left; discriminate]]]).
  discriminate H0.
  split; [discriminate|split;[omega|split;[omega|right]]].
...
Proof.
  unfold slot_within_bounds, slot_valid, index_valid; intros. 
  destruct ty; InvBooleans; try (split; 
   [discriminate|split; [assumption|split;[omega|left; discriminate]]]).
  inv H0.
  split; [discriminate|split; [assumption|split;[omega|]]]. 
   right; rewrite H0.
(*  apply Zdivides_trans with 16. exists 4; auto with zarith.*)
...
Lemma T128_idx_align:
  forall idx,
  index_valid idx -> type_of_index idx = T128 ->
  (16|offset_of_index fe idx).
Proof.
intros. 
unfold offset_of_index; destruct idx; try inv H0;
simpl in H; simpl in H1; subst t.
destruct H as [_ [_ [_ [H|H]]]].
 elim H; reflexivity. apply aligned_16_4x; auto. 
...
Proof.
  ...
  destruct (typ_eq (type_of_index idx) T128).
   rewrite e; simpl.
   apply T128_idx_align; auto.
  replace (align_chunk (chunk_of_type (type_of_index idx))) with 4.
  apply offset_of_index_aligned; auto. 
  destruct (type_of_index idx); try (reflexivity || congruence).
  exists m'; auto.
Qed.
...
Proof.
  ...
  intro; apply H.  apply mreg_seq_within_bounds with r; auto.
...
Opaque destroyed_at_call.
  ...
  destruct vl; auto. 
  apply IHl; auto.
  red; intros. destruct l0. 
   destruct (mreg_seq_dec a r).
    absurd (In r destroyed_at_call); auto.
    apply mreg_seq_destroyed_at_call with a; auto. symmetry in m; auto.
...
rewrite Locmap.gso. auto.
red; unfold mreg_seq.
destruct r; simpl; try discriminate. simpl in H; intuition.
destruct r; simpl; try discriminate. simpl in H; intuition.
Opaque destroyed_at_function_entry.
Qed.
(*
Remark LTL_undef_regs_others_ORIG:
  forall r rl ls, ~In r rl -> LTL.undef_regs rl ls (R r) = ls (R r).
Proof.
  induction rl; simpl; intros. auto.
  rewrite Locmap.gso. apply IHrl. intuition. red. intuition.
(* isto aqui parece que NÃO!!! --- é preciso fortalecer com ~In r' rl
 para todo r'~r 
OBS: só é usado com "destroyed_at_function_entry"
*)
adm. 
Qed.
*)
...
Remark LTL_undef_regs_slot:
  forall sl ofs ty rl ls,
    LTL.undef_regs rl ls (S sl ofs ty) = ls (S sl ofs ty).
Proof.
  induction rl; simpl; intros. auto.
...
Proof.
  ...
  exploit Mem.alloc_left_mapped_inject.
    ...
    red. intros. apply Zdivides_trans with 16. 
...
Proof.
  ...
  assert (slot_valid f Outgoing pos ty = true).
    exploit loc_arguments_acceptable; eauto. intros [A [B1 B2]]. 
    unfold slot_valid. unfold proj_sumbool. rewrite zle_true.
    destruct ty; reflexivity || (try congruence).
    simpl; destruct B2 as [B2|B2]. elim B2; reflexivity.
    destruct (zeq pos (align pos 4)); auto. elim n.
    apply divides_align_eq; auto. omega.
    omega.
...
Proof.
  ...
  (* Lgetstack, incoming *)
  assert (In (S Incoming ofs ty) (loc_parameters (Linear.fn_sig f))).
   unfold slot_valid in H0; destruct ty; InvBooleans; auto. congruence.
  ...
  exploit loc_arguments_acceptable; eauto. intros [A [B1 [B2|B2]]].
   unfold slot_valid, proj_sumbool. rewrite zle_true. 
   destruct ty; reflexivity || congruence. omega.
  unfold slot_valid, proj_sumbool. rewrite zle_true. 
  destruct ty; try (reflexivity || congruence). simpl.
  destruct (zeq ofs (align ofs 4)); auto.
  elim n. apply divides_align_eq; auto; omega. omega.
  intros [v [A B]].
  econstructor; split.
