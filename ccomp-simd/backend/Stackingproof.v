...
Definition index_valid (idx: frame_index) :=
  ...
  | FI_link => True
  | FI_retaddr => True
  | FI_local x ty => ty <> Tlong /\ 0 <= x /\ x + typesize ty <= b.(bound_local) /\ (align_chunk (chunk_of_type ty) | 4*x)
  | FI_arg x ty => ty <> Tlong /\ 0 <= x /\ x + typesize ty <= b.(bound_outgoing) /\ (align_chunk (chunk_of_type ty) | 4*x)
...
Proof.
  ...
  destruct ty; simpl in *; InvBooleans;
  try ( split; [discriminate|split;[omega|split; auto]]).
...
discriminate H0.
(*
  rewrite H0; apply align_divides.
  apply Archi.vec_align_chunk_pos.
*)
Qed.
...
Proof.
  unfold slot_within_bounds, slot_valid, index_valid; intros. 
  destruct ty; InvBooleans;
  (split; [discriminate|split; [auto|split;[omega|]]];
  try (discriminate H0 || rewrite H0; apply align_divides; simpl; omega)).
(*
  rewrite H0; apply align_divides; simpl; apply Archi.vec_align_chunk_pos.
*)
Qed.
...
Lemma offset_of_index_align8:
  forall idx,
  index_valid idx ->
  type_of_index idx = Tvec tt \/ type_of_index idx = Tfloat ->
  (8|offset_of_index fe idx).
Proof.
intros. 
unfold offset_of_index; destruct idx; destruct H0; try inv H0;
simpl in H.
 destruct H as [_ [_ [_ H]]].
simpl in H1; subst.
 elim H. simpl align_chunk; unfold Archi.vec_align_chunk.
 unfold fe; generalize (frame_env_aligned b); intuition. intuition.
simpl in H1; subst.
 elim H. simpl align_chunk; unfold Archi.vec_align_chunk.
 unfold fe; generalize (frame_env_aligned b); intuition. intuition.
simpl in H1; subst.
 elim H. simpl align_chunk; unfold Archi.vec_align_chunk.
 unfold fe; generalize (frame_env_aligned b); intuition. 
simpl in H1; subst.
 elim H. simpl align_chunk; unfold Archi.vec_align_chunk.
 unfold fe; generalize (frame_env_aligned b); intuition. 
 unfold fe; generalize (frame_env_aligned b); intuition. intuition.
...
case_eq (type_of_index idx); intro Hty.
  apply offset_of_index_aligned; auto.
  apply offset_of_index_align8; auto.
admit (* absurd *).
  apply offset_of_index_aligned; auto.
  intro HH; apply offset_of_index_align8; auto.
left; rewrite HH; f_equal; elim Hty; reflexivity.
...
(* SIMD *)
  generalize B; 
  case (Archi.vec_typ_eq v v); intro HH; [clear HH| elim HH; auto].
  intro HH; rewrite HH; econstructor; eauto.
(* eSIMD *)
Qed.
...
Proof.
  ...
  econstructor; eauto.
(* SIMD *)
  case (Archi.vec_typ_eq v t); intro HH.
   subst; generalize B; case (Archi.vec_typ_eq t t); intro HH;
   [|elim HH; econstructor; auto].
   clear HH; intro HH; rewrite HH; econstructor; eauto.
  auto.
(* eSIMD *)
Qed.
...
Lemma Locmap_gso':
  forall (l : loc) (v : val) (m : Locmap.t) (p : loc),
   l<>p -> Locmap.set l v m p = m p \/ Locmap.set l v m p = Vundef.
Proof.
intros.
unfold Locmap.set at 2.
destruct (Loc.diff_dec l p).
 left; rewrite Locmap.gso; auto.
right; destruct (Loc.eq l p); congruence.
Qed.
...
(* SIMD *)
  assert (R r <> R r0). intro HH; apply n; inv HH; reflexivity.
  generalize (Locmap_gso' (R r) v ls (R r0) H1). intros [A|A];
   rewrite A; auto.
...
Proof.
  ...
  rewrite Locmap.gso. auto. red. 
(* SIMD *)
(*
Lemma mreg_diff_within_bounds: forall b r1 r2,
 ~mreg_within_bounds b r1 -> mreg_within_bounds b r2 -> mreg_diff r1 r2.
Proof.
unfold mreg_within_bounds; intros bnds r1 r2.
destruct r1; destruct r2; intros H1 H2; vm_compute;
(reflexivity || elim H1); auto;
try (simpl in H2; simpl; omega).
(*
try (elim H; reflexivity); auto; simpl; intros [H1 H2];
...
(* mreg_diff_within_bounds ISSUE!!! also related with alignment...
  intro; apply H.  apply mreg_diff_within_bounds with r; auto.
...
(* SIMD 
  destruct (zlt (index_vec_callee_save r) 0).
 eSIMD *)
  generalize (bound_int_callee_save_pos b) (bound_float_callee_save_pos b).
admit(*  omega.*).
(* SIMD 
  exfalso. eapply vec_callee_save_not_destroyed; eauto. eapply index_vec_callee_save_pos2; eauto.
 eSIMD *)
...
Lemma agree_callee_save_set_result:
  forall sg vl ls1 ls2 lres,
  loc_result sg = Some lres ->
  agree_callee_save ls1 ls2 ->
  agree_callee_save (Locmap.setlist (map R lres) vl ls1) ls2.
Proof.
admit (*
...
(* SIMD *)
  destruct l0.
   ...
   absurd (In r destroyed_at_call); auto.
   apply mreg_diff_destroyed_at_call with a; auto. 
   red; intro HH; apply n; apply mreg_diff_sym; auto.
  rewrite Locmap.gso; auto.
(* rewrite Locmap.gso. apply H0; auto. 
...
(* FAZER ESTA PROVA DIREITO!!! (de forma abstracta) *)
Remark LTL_undef_regs_others:
  forall r ls,
    ~In r destroyed_at_function_entry ->
    LTL.undef_regs destroyed_at_function_entry ls (R r) = ls (R r).
Proof.
Transparent destroyed_at_function_entry.
unfold destroyed_at_function_entry.
intros; simpl.
rewrite Locmap.gso. auto.
destruct r; compute; try reflexivity.
elim H; simpl; left; reflexivity.
Opaque destroyed_at_function_entry.
Qed.
(*
Remark LTL_undef_regs_others:
...
Local Transparent destroyed_by_setstack.
  ...
  intros. simpl. split. apply Zge_le. apply index_float_callee_save_pos; auto.
clear A; admit. clear A; admit.
   simpl; auto.
  intros; congruence.
  intros; simpl. destruct idx; auto. congruence.
...
(*
  destruct (zlt (index_float_callee_save r) 0).
   generalize (bound_float_callee_save_pos b).
   destruct (zlt (index_vec_callee_save r) 0).
    omega. 
   split. omega.
(* int_vec_callee_save_disjoint
  eelim int_float_callee_save_disjoint. eauto. *) admit.
 split.
  ...
  eapply index_float_callee_save_pos2. eauto. auto.
   ...
   destruct (zlt (index_vec_callee_save r) 0).
    omega.
   intros.  *)
admit (* out pos disjointness float/vec_callee ou int/vec_callee *).
admit.
admit.
  apply incl_refl.
admit.
eauto.
intro A; clear A; admit.
...
(* SIMD *)
(* outra vez index_vec_callee_save... *)
admit.
elimtype False; clear A; admit.
(* eSIMD *)
  destruct H2; auto. 
clear A; admit (*  eapply agree_saved_float; eauto. *). clear A; admit.
clear A; admit.
  apply incl_refl.
clear A; admit (*  apply float_callee_save_norepet.*).
  eexact D.
  intros [rs2 [P [Q [R S]]]].
  exists rs2.
...
(*
    apply B. intuition. 
admit (* outra vez vec_callee_save *).
...
Proof.
  ...
  elim (H4 _ H).
  unfold parent_sp.
assert (forall x y, (x|y) -> (if zeq y (align y x) then true else false) = true). admit.
admit(*
  assert (slot_valid f Outgoing pos ty = true).
    exploit loc_arguments_acceptable; eauto. intros [A [B C]]. 
    unfold slot_valid. unfold proj_sumbool. rewrite zle_true. 
    destruct ty; try rewrite H1; auto. omega. 
...
+ (* Lgetstack, local *)
  ...
  apply agree_frame_set_reg; auto.
+ (* Lgetstack, incoming *)
  assert (In (S Incoming ofs ty) (loc_parameters (Linear.fn_sig f)) /\ ty <> Tlong) as [Hin].
  unfold slot_valid in SV. 
  destruct ty; InvBooleans; split; auto; try discriminate.
...
- (* external function *)
  ...
  inv H1; inv A.
  eapply match_stack_change_extcall; eauto. apply Ple_refl. apply Ple_refl. 
  eapply external_call_nextblock'; eauto.
  eapply external_call_nextblock'; eauto.
  apply agree_regs_set_regs; auto. apply agree_regs_inject_incr with j; auto. 
admit (*  apply agree_callee_save_set_result; auto. *).
