...
let mkef sg toks =
  ...
  | [EFT_tok "simd"; EFT_string s; EFT_tok "with"; EFT_lint imms] ->
      let id = intern_string s in
      begin try let blt = Hashtbl.find simd_builtin_functions id in
                EF_builtin(id, Some (mkBlt_data blt imms), sg)
            with Not_found -> raise Parsing.Parse_error
      end
  | [EFT_tok "simd"; EFT_string s] ->
      let id = intern_string s in
      begin try let blt = Hashtbl.find simd_builtin_functions id in
                if blt.bimmargs = 0
                then EF_builtin(id, Some (mkBlt_data blt []), sg)
                 else raise Parsing.Parse_error
            with Not_found -> raise Parsing.Parse_error
      end
  | [EFT_tok "builtin"; EFT_string s] ->
      EF_builtin(intern_string s, None, sg)
  | [EFT_tok "volatile"; EFT_tok "load"; EFT_chunk c] ->
      EF_vload c
  | [EFT_tok "volatile"; EFT_tok "store"; EFT_chunk c] ->
      EF_vstore c
  | [EFT_tok "volatile"; EFT_tok "load"; EFT_chunk c; 
     EFT_tok "global"; EFT_string s; EFT_lint [n]] ->
      EF_vload_global(c, intern_string s, coqint_of_camlint64 n)
  | [EFT_tok "volatile"; EFT_tok "store"; EFT_chunk c; 
     EFT_tok "global"; EFT_string s; EFT_lint [n]] ->
      EF_vstore_global(c, intern_string s, coqint_of_camlint64 n)
  | [EFT_tok "malloc"] ->
      EF_malloc
  | [EFT_tok "free"] ->
      EF_free
  | [EFT_tok "memcpy"; EFT_tok "size"; EFT_lint [sz]; EFT_tok "align"; EFT_lint [al]] ->
      EF_memcpy(Z.of_sint64 sz, Z.of_sint64 al)
...
let rec convert_rexpr = function
  | Rvar id -> Evar id
  | Rconst c -> Econst c
  | Runop(op, e1) -> let c1 = convert_rexpr e1 in Eunop(op, c1)
  | Rbinop(op, e1, e2) ->
      let c1 = convert_rexpr e1 in
      let c2 = convert_rexpr e2 in
      Ebinop(op, c1, c2)
  | Rload(chunk, e1) -> let c1 = convert_rexpr e1 in Eload(chunk, c1)
  | Rcall(sg, e1, el) ->
      raise Parsing.Parse_error
(*
      let c1 = convert_rexpr e1 in
      let cl = convert_rexpr_list el in
      let temps = mktemps (List.length sg.sig_res) in
      convert_accu := Scall(temps, sg, c1, cl) :: !convert_accu;
      List.map (fun x->Evar x) temps
*)
  | Rbuiltin(sg, pef, el) ->
      raise Parsing.Parse_error
(*
      ...
      let cl = convert_rexpr_list el in
      let temps = mktemps (List.length sg.sig_res) in
      convert_accu := Sbuiltin(temps, ef, cl) :: !convert_accu;
      List.map (fun x->Evar x) temps
*)
...
let mkeval e =
  ...
  | Rcall(sg, e1, el) ->
      let c1 = convert_rexpr e1 in
      let cl = convert_rexpr_list el in
      prepend_seq !convert_accu (Scall([], sg, c1, cl))
  | Rbuiltin(sg, pef, el) -> 
      let ef = mkef sg pef in
      let cl = convert_rexpr_list el in
      prepend_seq !convert_accu (Sbuiltin([], ef, cl))
  | _ ->
...
let mkassign ids e =
  ...
  | Rcall(sg, e1, el) ->
      let c1 = convert_rexpr e1 in
      let cl = convert_rexpr_list el in
      prepend_seq !convert_accu (Scall(ids, sg, c1, cl))
  | Rbuiltin(sg, pef, el) ->
      let ef = mkef sg pef in
      let cl = convert_rexpr_list el in
      prepend_seq !convert_accu (Sbuiltin(ids, ef, cl))
  | _ ->
      let c = convert_rexpr e in
      match ids with
      | id::[] -> prepend_seq !convert_accu (Sassign(id, c))
...
let mkreturn_some el =
  convert_accu := [];
  let cl = convert_rexpr_list el in
  prepend_seq !convert_accu (Sreturn (cl))
...
init_data:
   ...
   | FLOAT32 FLOATXLIT                            { Init_float32 (coqfloat_of_camlint64 $2) }
   | FLOAT64 FLOATLIT                            { Init_float64 (coqfloat_of_camlfloat $2) }
   | FLOAT64 FLOATXLIT                            { Init_float64 (coqfloat_of_camlint64 $2) }
   | FLOAT FLOATLIT                              { Init_float64 (coqfloat_of_camlfloat $2) }
   | FLOAT FLOATXLIT                              { Init_float64 (coqfloat_of_camlint64 $2) }
   | FLOATLIT                                    { Init_float64 (coqfloat_of_camlfloat $1) }
   | FLOATXLIT                                    { Init_float64 (coqfloat_of_camlint64 $1) }
...
signature:
    type_tuple 
               { {sig_args = []; sig_res = $1; sig_cc = cc_default} }
  | VOID
               { {sig_args = []; sig_res = []; sig_cc = cc_default} }
...
stmt:
  ...
  | parameter_list EQUAL expr SEMICOLON         { mkassign ($1) $3 }
  ...
  | RETURN SEMICOLON                            { Sreturn [] }
  | RETURN expr_list_1 SEMICOLON                { mkreturn_some $2 }
...
expr:
  ...
  | FLOATXLIT                                   { Rconst(Ofloatconst (coqfloat_of_camlint64 $1)) }
...
type_tuple:
    type_                                       { $1 :: [] }
  | type_ STAR type_tuple                       { $1 :: $3 }

/* External functions */

intlit_list1:
  | LONGLIT					{ $1::[] }
  | LONGLIT COMMA intlit_list1			{ $1 :: $3 }
  
eftok:
    IDENT                                       { EFT_tok $1 }
  | STRINGLIT                                   { EFT_string $1 }
  | intlit_list1				{ EFT_lint $1 }
(*  | INTLIT                                      { EFT_int $1 } *)
