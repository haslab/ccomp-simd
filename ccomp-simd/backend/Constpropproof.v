...
Opaque builtin_strength_reduction.
  ...
  (* Ireturn *)
  exploit Mem.free_parallel_extends; eauto. intros [m2' [A B]].
  left; exists O; exists (Returnstate s' (*regmap_optget or Vundef rs'*)rs'##or m2'); split.
  ...
  (* return *)
  assert (X: exists bc ae, ematch bc (regmap_setlist res vres Vundef rs) ae).
  { inv SS2. exists bc0; exists ae; auto. }
  destruct X as (bc1 & ae1 & MATCH).
  inv H4. inv H1. 
  left; exists O; econstructor; split.
  eapply exec_return; eauto. 
  econstructor; eauto. constructor. apply set_regs_lessdef; auto. 
Qed.
