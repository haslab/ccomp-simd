...
Fixpoint list_take {A: Type} (n: nat) (x: list A) {struct n} : list A :=
  match n with
  | O => nil
  | S n' => match x with nil => nil | hd :: tl => hd::list_take n' tl end
  end.
...
Inductive wt_instr : instruction -> Prop :=
  ...
  | wt_Icall:
      ...
      map env res = list_take (length res) (normalize_list (sig_res sig)) ->
      valid_successor s ->
...
*)
  | wt_Ireturn_some:
      forall arg tyl,
      funct.(fn_sig).(sig_res) = tyl ->
      map env arg = normalize_list tyl ->
      wt_instr (Ireturn (arg)).
...
Definition type_instr (e: S.typenv) (i: instruction) : res S.typenv :=
  ...
  | Iop op args res s =>
      ...
      if is_move op then
       ...
       (let (targs, tres) := type_of_operation op in
        do e1 <- S.set_list e args targs; 
        do e2 <- S.set e1 res (normalize tres);
        (* enforce Tsingle on return type *)
        S.adjust e2 res tres)
  | Iload chunk addr args dst s =>
      do x <- check_successor s;
      do e1 <- S.set_list e args (type_of_addressing addr);
      do e2 <- S.set e1 dst (normalize (type_of_chunk chunk));
      match type_of_chunk chunk with
      | Tsingle => S.adjust e2 dst Tsingle
      | _ => OK e2
      end
  | Istore chunk addr args src s =>
      do x <- check_successor s;
      do e1 <- S.set_list e args (type_of_addressing addr);
      do e2 <- S.set e1 src (type_of_chunk_use chunk);
      match type_of_chunk chunk with
      | Tsingle => S.adjust e2 src Tsingle
      | _ => OK e2
      end
  | Icall sig ros args res s =>
      ...
      S.set_list e2 res (list_take (length res) (normalize_list (sig_res sig)))
  | Itailcall sig ros args =>
      do e1 <- type_ros e ros;
      do e2 <- S.set_list e1 args (normalize_list sig.(sig_args));
      if list_eq_dec typ_eq sig.(sig_res) (sig_res f.(fn_sig)) then
  ...
  | Ibuiltin ef args res s =>
      ...
      S.set_list e1 res (list_take (length res) (normalize_list (sig_res sig)))
 ...
 | Ijumptable arg tbl =>
  ...
  | Ireturn optres => S.set_list e optres (normalize_list f.(fn_sig).(sig_res))
(*
...
- (* tailcall *)
  destruct (list_eq_dec typ_eq (sig_res s) (sig_res (fn_sig f))); try discriminate.
...
- (* return *)
  simpl in H. (*destruct o as [r|] eqn: RET;*) destruct (sig_res (fn_sig f)) as [t|] eqn: RES; try discriminate.
  econstructor. eauto. eapply S.set_list_sound; eauto with ty.
  inv H. econstructor; auto. 
  rewrite RES; eapply S.set_list_sound; eauto.
Qed.
*)
...
- (* call *)
  ...
  exploit S.set_list_complete. eauto. apply H1. intros [e2 [C D]].
  exploit S.set_list_complete. eexact D. apply H2. intros [e3 [E F]].
...
- (* builtin *)
  exploit S.set_list_complete. eauto. apply H0. intros [e1 [A B]].
  exploit S.set_list_complete. eexact B. apply H1. intros [e2 [C D]].
  exists e2; split; auto.
  rewrite check_successor_complete by auto; simpl. 
  rewrite A; simpl; rewrite C; auto.
- (* cond *)
...
- (* return some *)
  rewrite H0. apply S.set_list_complete; auto.
Qed.
*)
...
Proof.
  intros. destruct ty; try destruct v; reflexivity.
Qed.

Lemma normalize_subtype_list:
  forall tys, subtype_list tys (normalize_list tys) = true.
Proof.
induction tys; simpl; intros; auto.
rewrite IHtys, normalize_subtype; auto.
Qed.
...
Lemma wt_regset_assign2_list:
  forall env rs r v tys,
  wt_regset env rs ->
  Val.has_type_list v (map env r) ->
  map env r = normalize_list tys ->
  wt_regset env (regmap_setlist r v Vundef rs).
Proof.
induction r; simpl; intros; auto.
destruct v; try inv H0.
destruct tys; try inv H1.
eapply wt_regset_assign2; eauto.
rewrite H4; destruct t; auto.
Qed.
...
Lemma wt_exec_Ibuiltin:
  ...
  wt_regset env rs ->
  wt_regset env (regmap_setlist res vres Vundef rs).
Proof.
admit (*
  intros. inv H. 
  eapply wt_regset_assign2_list; eauto.
  eapply Val.has_subtype_list.
  rewrite H7; apply normalize_subtype_list.
...
Inductive wt_stackframes: list stackframe -> signature -> Prop :=
  ...
  | wt_stackframes_cons:
      ...
      wt_function f env ->
      wt_regset env rs ->
      map env res = normalize_list (sig_res sg) ->
...
Inductive wt_state: state -> Prop :=
  ...
  | wt_state_return:
      forall s v m sg,
      wt_stackframes s sg ->
      Val.has_type_list v (normalize_list (sig_res sg)) ->
...
- econstructor; eauto. rewrite H3, H; auto.
Qed.
...
admit. 
  ...
  (* Ireturn *)
  econstructor; eauto. 
  inv WTI; simpl. (*auto. unfold proj_sig_res; rewrite H2.*) rewrite <- H3.
  ...
  eapply Val.has_subtype_list. apply normalize_subtype_list. 
  eapply external_call_well_typed; eauto.
  (* return *)
  inv H1. econstructor; eauto.
  eapply wt_regset_assign2_list; eauto. rewrite H10; auto.
Qed.
