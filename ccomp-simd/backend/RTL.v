...
Inductive instruction: Type :=
  ...
  | Icall: signature -> reg + ident -> list reg -> list reg -> node -> instruction
  ...
  | Ibuiltin: external_function -> list reg -> list reg -> node -> instruction
...
Inductive stackframe : Type :=
  | Stackframe:
      forall (res: list reg)            (**r where to store the result *)
...
Inductive state : Type :=
  ...
  | Returnstate:
      forall (stack: list stackframe) (**r call stack *)
             (v: list val)                 (**r return value for the call *)
             (m: mem),                (**r memory state *)
      state.
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_Ibuiltin:
      ...
      step (State s f sp pc rs m)
         t (State s f sp pc' (regmap_setlist res v Vundef rs) m')
  | exec_Icond:
  ...
  | exec_return:
      ...
      step (Returnstate (Stackframe res f sp pc rs :: s) vres m)
        E0 (State s f sp pc (regmap_setlist res vres Vundef rs) m).
...
Inductive final_state: state -> int -> Prop :=
  | final_state_intro: forall r m,
      final_state (Returnstate nil (Vint r::nil) m) r.
...
(* receptiveness *)
  ...
  exploit external_call_receptive; eauto. intros [vres2 [m2 [EC2 _]]]. 
  exists (State s0 f sp pc' (regmap_setlist res vres2 Vundef rs) m2). eapply exec_Ibuiltin; eauto.
  exploit external_call_receptive; eauto. intros [vres2 [m2 [EC2 _]]]. 
...
*)
Definition instr_defs (i: instruction) : list reg :=
  match i with
  | Inop s => nil
  | Iop op args res s => res::nil
  | Iload chunk addr args dst s => dst::nil
  | Istore chunk addr args src s => nil
  | Icall sig ros args res s => res
  | Itailcall sig ros args => nil
  | Ibuiltin ef args res s => res
  | Icond cond args ifso ifnot => nil
  | Ijumptable arg tbl => nil
  | Ireturn optarg => nil
  end.
...
Definition max_reg_instr (m: positive) (pc: node) (i: instruction) :=
  ...
  | Icall sig (inl r) args res s => fold_left Pmax args (fold_left Pmax res (Pmax r m))
  | Icall sig (inr id) args res s => fold_left Pmax args (fold_left Pmax res m)
  | Itailcall sig (inl r) args => fold_left Pmax args (Pmax r m)
  | Itailcall sig (inr id) args => fold_left Pmax args m
  | Ibuiltin ef args res s => fold_left Pmax args (fold_left Pmax res m)
  | Icond cond args ifso ifnot => fold_left Pmax args m
  | Ijumptable arg tbl => Pmax arg m
(*  | Ireturn None => m *)
  | Ireturn (arg) => fold_left Pmax arg m
  end.
...
Proof.
  ...
  { induction l; simpl; intros. 
    auto.
    apply IHl. xomega. }
  destruct i; simpl; try (destruct s0); try (apply X); try (apply X); try xomega.
(*  destruct o; xomega.*)
Qed.

Remark max_reg_instr_def:
  forall m pc i rl, instr_defs i = rl -> Ple (fold_left Pmax rl 1%positive) (max_reg_instr m pc i).
Proof.
  intros.
  assert (X: forall l n, Ple (fold_left Pmax rl 1%positive) n -> Ple (fold_left Pmax rl 1%positive) (fold_left Pmax l n)).
  { induction l; simpl; intros. xomega. apply IHl. xomega. }
  assert (Y: forall rl x y, Ple x y -> Ple (fold_left Pos.max rl x) (fold_left Pos.max rl y)).
  { induction rl0; simpl; intros; auto. apply IHrl0. xomega. }
  destruct i; simpl in *; inv H.
- simpl; xomega.
- apply X. simpl; xomega.
- apply X. simpl; xomega.
- apply X. simpl; xomega.
- destruct s0; apply X; apply Y; xomega.
- destruct s0; apply X; simpl; xomega. 
- apply X; apply Y; xomega.
- apply X; simpl; xomega.
- simpl; xomega.
- simpl; xomega.
Qed.
...
- destruct H. right; subst; apply X. right; xomega. left; auto.
- destruct H. right; subst; xomega. auto.
...
Lemma max_reg_function_def:
  forall f pc i rl,
  f.(fn_code)!pc = Some i -> instr_defs i = rl -> Ple (fold_left Pmax rl 1%positive) (max_reg_function f).
Proof.
  intros. 
  assert (Ple (fold_left Pmax rl 1%positive) (PTree.fold max_reg_instr f.(fn_code) 1%positive)).
  {  revert H. 
     apply PTree_Properties.fold_rec with
       (P := fun c m => c!pc = Some i -> Ple (fold_left Pmax rl 1%positive) m).
