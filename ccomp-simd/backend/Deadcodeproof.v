...
Inductive match_stackframes: stackframe -> stackframe -> Prop :=
  | match_stackframes_intro:
      forall res f sp pc e tf te an
        ...
        (RES: forall v tv,
              Val.lessdef_list v tv ->
              eagree (regmap_setlist res v Vundef e) (regmap_setlist res tv Vundef te)
...
Lemma transf_volatile_store:
  ...
  v = nil /\
  exists tm', volatile_store_sem chunk ge (v1'::v2'::nil) tm t nil tm'
           /\ magree m' tm' (nlive ge sp nm).
Proof.
...
Lemma eagree_set_undef_list: forall l e1 e2 ne,
  eagree e1 e2 ne -> eagree (regmap_setlist l nil Vundef e1) e2 ne.
Proof.
induction l; simpl; intros; auto.
apply eagree_set_undef; apply IHl; auto.
Qed.

Lemma eagree_update_list:
  forall r e1 e2 ne v1 v2,
  Val.lessdef_list v1 v2 (*vagree v1 v2 (nreg ne r)*) ->
  eagree e1 e2 (fold_left (fun x r=>NE.set r Nothing x) r ne) ->
  eagree (regmap_setlist r v1 Vundef e1) (regmap_setlist r v2 Vundef e2) ne.
Proof.
induction r; simpl; intros; auto.
destruct v1.
 inv H. unfold eagree. intros. 
 apply eagree_update.
  apply vagree_same.
 apply IHr; auto.
inv H.
...
- (* call *)
  ...
  apply eagree_update_list; eauto with na.
...
+ (* volatile load *)
  ...
  assert (X: exists tv, volatile_load ge chunk tm b ofs t tv /\ Val.lessdef v0 tv).
  {
    inv H2. 
  ...
  * exploit magree_load; eauto. 
  ...
  eapply match_succ_states; eauto. simpl; auto.
  apply eagree_update_list; eauto 2 with na.
  eapply magree_monotone; eauto. intros. apply incl_nmem_add; auto. 
...
+ (* volatile store *)
  ...
  exact symbols_preserved. exact varinfo_preserved.
  eapply match_succ_states; eauto. simpl; auto.
  apply eagree_update_list; eauto 3 with na.
...
apply eagree_set_undef_list; auto.
...
Proof.
  intros. inv H0. inv H. inv STACKS. inv RES. inv H3; inv H1; constructor. 
Qed.
