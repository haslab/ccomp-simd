...
*)
Lemma alloc_optreg_valid:
  forall dest s1 s2 map rl i,
  map_valid map s1 ->
  alloc_optreg map dest s1 = OK rl s2 i -> regs_valid rl s2.
Proof.
 induction dest; simpl; intros.
  inv H0; apply regs_valid_nil.
(*apply bind_inversion in H0; destruct H0 as [r [sr1 [sr2 [ir [Hr1 Hr2]]]]].
  inv Hr2; apply regs_valid_cons; [|apply regs_valid_nil].
  eapply new_reg_valid; eauto.*)
 apply bind_inversion in H0; destruct H0 as [r [sr1 [sr2 [ir [Hr1 Hr2]]]]].
 apply bind_inversion in Hr2; destruct Hr2 as [rs [srs1 [srs2 [irs [Hrs1 Hrs2]]]]].
 inv Hrs2. clear irs.
 apply regs_valid_cons.
  eapply reg_valid_incr; eauto.
  eapply reg_valid_incr; eauto.
  eapply find_var_valid; eauto.
 apply (IHdest sr1 _ map _ srs2); auto.
 eapply map_valid_incr; eauto.
Qed.
Hint Resolve alloc_optreg_valid: rtlg.

Lemma alloc_optreg_fresh_or_in_map:
  forall map dest s rl s' i r,
  map_valid map s ->
  alloc_optreg map dest s = OK rl s' i ->
  In r rl ->
  reg_in_map map r \/ reg_fresh r s.
Proof.
induction dest; simpl; intros; inv H0.
 inv H1.
(* apply bind_inversion in H0; destruct H0 as [r' [sr1 [sr2 [ir [Hr1 Hr2]]]]].
 inv Hr2; inv H1.
  right; eapply new_reg_fresh; eauto.
 inv H0.*)
apply bind_inversion in H3; destruct H3 as [r' [sr1 [sr2 [ir [Hr1 Hr2]]]]].
apply bind_inversion in Hr2; destruct Hr2 as [rs [srs1 [srs2 [irs [Hrs1 Hrs2]]]]].
inv Hrs2. clear irs. inv H1.
 left; eauto with rtlg.
assert (X: reg_in_map map r \/ reg_fresh r sr1).
 eapply (IHdest _ rs ); eauto.
 eapply map_valid_incr; eauto.
destruct X; [left|right]; auto.
eapply reg_fresh_decr; eauto.
Qed.
...
*)
Inductive return_reg_ok: state -> mapping -> list reg -> Prop :=
  | return_reg_ok_nil:
      forall s map,
      return_reg_ok s map nil
  | return_reg_ok_cons:
      forall s map r rl,
      ~(reg_in_map map r) -> reg_valid r s ->
      return_reg_ok s map rl ->
      return_reg_ok s map (r::rl).
...
(** [regs_map_ok map rl ids] characterizes the destination registers
  for a multi-value expression: if [ids] is [nil], the destination is
  a single fresh register (not associated with any variable);
  if [ids] is non-empty, the destination are the registers
  associated with local variables [ids]. *)

Inductive reglist_map_ok: mapping -> list reg -> list ident -> Prop :=
  | regs_map_ok_nil: forall map,
      reglist_map_ok map nil nil
  | reglist_map_ok_cons: forall map r rd id ids,
      map.(map_vars)!id = Some r ->
      reglist_map_ok map rd ids ->
      reglist_map_ok map (r::rd) (id::ids).

Inductive regs_map_ok: mapping -> list reg -> list ident -> Prop :=
  | regs_map_ok_novar: forall map r,
       ~reg_in_map map r ->
      regs_map_ok map (r::nil) nil
  | regs_map_ok_vars: forall map rd ids,
      reglist_map_ok map rd ids ->
      regs_map_ok map rd ids.
...
Inductive tr_stmt (c: code) (map: mapping):
     stmt -> node -> node -> list node -> labelmap -> node -> list reg -> Prop :=
...
(*     tr_expr c map nil a ns nret rret None ->*)
     tr_exprlist c map nil a ns nret rret ->
     tr_stmt c map (Sreturn a) ns nd nexits ngoto nret rret
...
Inductive tr_function: CminorSel.function -> RTL.function -> Prop :=
  | tr_function_intro:
      forall f code rparams map1 s0 s1 i1 rvars map2 s2 i2 nentry ngoto nret orret,
...
Lemma alloc_optreg_map_ok:
  ...
  regs_map_ok map r optid.
Proof.
induction optid.
 simpl; intros; constructor. simpl.

admit (* !!!
  unfold alloc_optreg; intros. destruct optid.
   apply bind_inversion in H0; destruct H0 as [r' [sr1 [sr2 [ir [Hr1 Hr2]]]]].
   inv Hr2.
   constructor 1. eapply new_reg_not_in_map; eauto.
  apply bind_inversion in H0; destruct H0 as [r' [sr1 [sr2 [ir [Hr1 Hr2]]]]].
  apply bind_inversion in Hr2; destruct Hr2 as [rl [sr1' [sr2' [ir' [Hr1' Hr2']]]]]. inv Hr2'.
destruct optid.
 unfold new_reg in Hr1. find_var in H0. destruct (map_vars map)!i0; monadInv H0. auto.
