...
*)
Fixpoint alloc_optreg (map: mapping) (dest: list ident) : mon (list reg) :=
  match dest with
  | nil => ret (nil)
  | id::l => do r <- find_var map id;
             do rl <- alloc_optreg map l;
             ret (r::rl)
  end.
...
Fixpoint transl_expr (map: mapping) (a: expr) (rd: reg) (nd: node)
  ...
  | Ebuiltin ef al =>
      do rl <- alloc_regs map al;
      do no <- add_instr (Ibuiltin ef rl (rd::nil) nd);
      transl_exprlist map al rl no
  | Eexternal id sg al =>
      do rl <- alloc_regs map al;
      do no <- add_instr (Icall sg (inr id) rl (rd::nil) nd);
      transl_exprlist map al rl no
  end
...
Fixpoint transl_stmt (map: mapping) (s: stmt) (nd: node)
                     (nexits: list node) (ngoto: labelmap) (nret: node) (rret: list reg)
                     {struct s} : mon node :=
  match s with
  | Sskip =>
      ret nd
  ...
  | Sreturn opt_a => transl_exprlist map opt_a rret nret
(*
...
*)
Fixpoint ret_regs (sigres: list typ) : mon (list reg) :=
  match sigres with
  | nil => ret nil
  | ty::l => do r <- new_reg;
             do rl <- ret_regs l;
...
*)do orret <- ret_regs f.(CminorSel.fn_sig).(sig_res);
