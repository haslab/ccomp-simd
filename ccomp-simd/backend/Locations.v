...
Lemma mreg_conflict_refl (r: mreg): mreg_conflict r r = true.
Proof.
Admitted.

Lemma mreg_conflict_sym (r1 r2: mreg):
 mreg_conflict r1 r2 = mreg_conflict r2 r1.
Proof.
unfold mreg_conflict, mreg_bank_conflict.
Admitted.
  ...
  Proof.
    ...
    rewrite mreg_conflict_refl; auto.
  ...
  Proof.
    ...
    rewrite mreg_conflict_sym; auto.
    intuition.
  Qed.
  ...
  - destruct (mreg_conflict r r0); [right|left]; auto.
  - left; auto.
  - left; auto.
...
Definition vec_typ_index vty : positive :=
 match vty with
 | Archi.T128 => 1%positive
 | T256 => 2%positive
 end.
...
Module IndexedTyp <: INDEXED_TYPE.
  ...
  Definition index (x: t) :=
    ...
    | Tfloat => 5%positive
    | Tlong => 6%positive
    | Tvec vv => (6+vec_typ_index vv)%positive
    end.
  Lemma index_inj: forall x y, index x = index y -> x = y.
  Proof. 
  admit. (*destruct x; destruct y; simpl; congruence. Qed.*)
  Admitted.
...
admit.(*
 assert (IndexedMreg.index mr <> IndexedMreg.index mr').
      ...
      congruence.
*)
    - admit (*assert (RANGE: forall ty, 1 <= typesize ty <= 2).
