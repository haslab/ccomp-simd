...
inv B. inv H9.
  exists (rs1#rd <- v2); exists tm2.
(* Exec *)
  split. eapply star_right. eexact EX1.
replace (rs1 # rd <- v2) with (regmap_setlist (rd::nil) (v2::nil) Vundef rs1).
...
Inductive tr_fun (tf: function) (map: mapping) (f: CminorSel.function)
                     (ngoto: labelmap) (nret: node) (rret: list reg) : Prop :=
...
Inductive tr_cont: RTL.code -> mapping -> 
                   CminorSel.cont -> node -> list node -> labelmap -> node -> list reg ->
...
admit.
  ...
  traceEq. 
  econstructor; eauto. constructor.
admit (*  eapply match_env_update_dest; eauto.*).
...
Proof.
  intros. inv H0. inv H. inv MS. inv LD. 
inv H3; inv H1.
  constructor.
Qed.
