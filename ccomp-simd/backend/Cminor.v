...
Inductive stmt : Type :=
  ...
  | Scall : list ident -> signature -> expr -> list expr -> stmt
  | Stailcall: signature -> expr -> list expr -> stmt
  | Sbuiltin : list ident -> external_function -> list expr -> stmt
  | Sseq: stmt -> stmt -> stmt
...
Fixpoint set_ret (ids: list ident) (vs: list val) (e: env) : env :=
  match ids, vs with
  | nil, _ => e
  | i1 :: is, v1::l => PTree.set i1 v1 (set_ret is l e)
  | i1 :: is, nil => PTree.set i1 Vundef (set_ret is nil e)
  end.
...
Inductive cont: Type :=
  ...
  | Kcall: list ident -> function -> val -> env -> cont -> cont.
                                        (**r return to caller *)
...
Inductive state: Type :=
  ...
  | Returnstate:                (**r Return from a function *)
      forall (v: list val)                   (**r Return value *)
             (k: cont)                  (**r what to do next *)
             (m: mem),                  (**r memory state *)
      state.
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | step_builtin: forall f optid ef bl k sp e m vargs t vres m',
      ...
      step (State f (Sbuiltin optid ef bl) k sp e m)
         t (State f Sskip k sp (set_ret optid vres e) m')
...
(*
  | step_return_0: forall f k sp e m m',
      Mem.free m sp 0 f.(fn_stackspace) = Some m' ->
      step (State f (Sreturn nil) k (Vptr sp Int.zero) e m)
        E0 (Returnstate (Vundef::nil) (call_cont k) m')
*)
  | step_return_1: forall f a k sp e m v m',
      eval_exprlist (Vptr sp Int.zero) e m a v ->
      Mem.free m sp 0 f.(fn_stackspace) = Some m' ->
      step (State f (Sreturn a) k (Vptr sp Int.zero) e m)
  ...
  | step_return: forall v optid f sp e k m,
      step (Returnstate v (Kcall optid f sp e k) m)
        E0 (State f Sskip k sp (set_ret optid v e) m).
...
(* receptiveness *)
  ...
  exists (State f Sskip k sp (set_ret optid vres2 e) m2). econstructor; eauto. 
  exploit external_call_receptive; eauto. intros [vres2 [m2 [EC2 _]]]. 
...
Inductive outcome: Type :=
  ...
  | Out_return: list val -> outcome  (**r return immediately to caller *)
  | Out_tailcall_return: list val -> outcome. (**r already returned to caller via a tailcall *)
...
Definition outcome_result_value
    (out: outcome) (retsig: list typ) (vres: list val) : Prop :=
  match out with
  | Out_normal => vres = (Vundef::nil)
(*  | Out_return None => vres = Vundef *)
  | Out_return v => (*retsig <> None /\*) vres = v
...
*)
  | exec_Sreturn_some:
      ...
      exec_stmt f sp e m (Sreturn a) E0 e m (Out_return v)
  | exec_Stailcall:
...
Inductive bigstep_program_terminates (p: program): trace -> int -> Prop :=
  | bigstep_program_terminates_intro:
      ...
      Genv.find_funct_ptr ge b = Some f ->
      funsig f = signature_main ->
      eval_funcall ge m0 f nil t m (Vint r::nil) ->
