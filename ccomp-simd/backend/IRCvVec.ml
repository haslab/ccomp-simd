...
let interfering_classes c1 c2 = c1=c2 || c1>0 && c2>0

let type_of_class c =
  if c = 0 then Tint else if c = 1 then Tfloat else Tvec ()
...
let init costs =
  ...
  and vec_caller_save = remove_reserved vec_caller_save_regs
  and int_callee_save = remove_reserved int_callee_save_regs
  and float_callee_save = remove_reserved float_callee_save_regs
  and vec_callee_save = remove_reserved vec_callee_save_regs in
  {
    caller_save_registers =
      [| Array.of_list int_caller_save;
	 Array.of_list float_caller_save;
	 Array.of_list vec_caller_save |];
    callee_save_registers =
      [| Array.of_list int_callee_save;
	 Array.of_list float_callee_save;
	 Array.of_list vec_callee_save |];
    num_available_registers =
      [| List.length int_caller_save + 
	   List.length int_callee_save;
         List.length float_caller_save +
	   List.length float_callee_save;
         List.length vec_caller_save +
	   List.length vec_callee_save |];
    start_points =
      [| 0; 0; 0 |];
    allocatable_registers =
      int_caller_save @ int_callee_save @ float_caller_save @ float_callee_save @ vec_caller_save @ vec_callee_save;
    stats_of_reg = costs;
...
let recordInterf n1 n2 =
  match n2.color with
  | None | Some (R _) ->
      if interfering_classes n1.regclass n2.regclass then begin
...
let selectSpill g =
  ...
  if !Clflags.option_dalloctrace then printf "Spilling %s\n" (name_of_node n);
  freezeMoves g n;
  n.nstate <- SelectStack;
  iterAdjacent (decrementDegree g) n;
  n
...
let regset_interf (rs:int64*int64) r =
  let (b,ridx) = mreg_idx r in
...
let find_reg g conflicts regclass ty =
  let rec find avail curr last =
    ...
    then None
      ...
      let r = avail.(curr) in
      if regset_interf conflicts r (* Regset.mem r conflicts*)
...
let record_reg_conflict cnf n =
  ...
  | Some (R r) -> regset_add cnf r (* Regset.add r cnf *)
  | _ -> cnf
