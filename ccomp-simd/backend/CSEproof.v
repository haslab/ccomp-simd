...
(* SIMD *)
- destruct (Archi.vec_typ_eq v0 t); auto.
(* eSIMD *)
Qed.
...
Lemma set_regs_lessdef:
  forall r v1 v2 rs1 rs2,
  Val.lessdef_list v1 v2 -> regs_lessdef rs1 rs2 -> regs_lessdef (regmap_setlist r v1 Vundef rs1) (regmap_setlist r v2 Vundef rs2).
Proof.
...
Inductive match_stackframes: list stackframe -> list stackframe -> Prop :=
  ...
  | match_stackframes_cons:
      forall res sp pc rs f approx s rs' s'
           ...
           (SAT: forall v m, exists valu, numbering_holds valu ge sp (regmap_setlist res v Vundef rs) m approx!!pc)
...
* unfold transfer; rewrite H.
  ...
  assert (CASE1: exists valu, numbering_holds valu ge sp (regmap_setlist res v Vundef rs) m' empty_numbering).
  { exists valu; apply empty_numbering_holds. }
  assert (CASE2: m' = m -> exists valu, numbering_holds valu ge sp (regmap_setlist res v Vundef rs) m' (fold_right (fun r x=>set_unknown x r) approx#pc res)).
  { intros. rewrite H1. exists valu.
    generalize v; elim res; simpl; intros; auto.
    destruct v0; auto; apply set_unknown_holds; auto. }
  assert (CASE3: exists valu, numbering_holds valu ge sp (regmap_setlist res v Vundef rs) m'
                         (fold_right (fun r x=>set_unknown x r) (kill_all_loads approx#pc) res)).
  { exists valu. 
    generalize v; elim res; simpl; intros; auto.
...
- (* Ireturn *)
  ...
  econstructor; eauto.
  apply regs_lessdef_regs; auto. (*  destruct or; simpl; auto. *)
...
Proof.
  intros. inv H0. inv H. inv H5. inv H3. inv H4; inv H1; constructor. 
Qed.
