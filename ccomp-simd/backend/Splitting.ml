...
let reg_list_dead rl map = List.fold_right (reg_dead) rl map
...
let ren_instr f maps pc i =
  ...
  | Icall(sg, ros, args, res, s) ->
      Icall(sg, ren_ros before ros, ren_regs before args, ren_regs after res, s)
  | Itailcall(sg, ros, args) ->
      Itailcall(sg, ren_ros before ros, ren_regs before args)
  | Ibuiltin(ef, args, res, s) ->
      Ibuiltin(ef, ren_regs before args, ren_regs after res, s)
  | Icond(cond, args, s1, s2) ->
      Icond(cond, ren_regs before args, s1, s2)
  | Ijumptable(arg, tbl) ->
      Ijumptable(ren_reg before arg, tbl)
  | Ireturn optarg ->
      Ireturn((ren_regs before) optarg)
