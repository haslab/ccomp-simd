...
Fixpoint ana_code (lm: labelmap) (ch: bool) (fs: singlefloats) (c: code) : labelmap * bool :=
  ...
  | Lbuiltin ef args res :: c =>
      ana_code lm ch (setregs fs res (expand_longs (sig_res (ef_sig ef)))) c
  | Lannot ef args :: c =>
      ana_code lm ch fs c
  | Llabel lbl :: c =>
...
Definition slot_valid (sl: slot) (ofs: Z) (ty: typ): bool :=
  ...
  match ty with
  | Tint | Tfloat | Tsingle | Tvec _ => true
  | Tlong => false
  end.
*)
Definition slot_valid (sl: slot) (ofs: Z) (ty: typ): bool :=
  ...
  | Tvec _, _ => false
  | t, Local => zle 0 ofs
                && zeq (4*ofs) (align (4*ofs) (align_chunk (chunk_of_type t)))
  | t, Outgoing => zle 0 ofs
                && zeq (4*ofs) (align (4*ofs) (align_chunk (chunk_of_type t)))
  | _, Incoming => In_dec Loc.eq (S Incoming ofs ty) (loc_parameters funct.(fn_sig))
  end.
(* eSIMD *)
...
Fixpoint wt_code (fs: singlefloats) (c: code) : bool :=
  ...
  | Ltailcall sg ros :: c =>
      ...
      wt_code None c
  | Lbuiltin ef args res :: c =>
      let ty_res := expand_longs (sig_res (ef_sig ef)) in
...
Lemma callregs_setregs_result:
  ...
  FSincl (setregs fs (lres) (expand_longs (sig_res sg))) (callregs fs).
Proof.
  ...
  assert (Y: forall rl s tyl,
             exists s', setregs (Some s) rl (expand_longs tyl) = Some s'
                     /\ forall l, Locset.In l s -> ~In l (map R rl) -> Locset.In l s').
  { admit (* isto já não dá para provar por indução em rl...
    induction rl; simpl; intros.
  - exists s; split. destruct tyl; auto. 
    destruct (expand_longs (t :: tyl)); simpl; auto.
    tauto.
  - destruct tyl. exists s; tauto. 
    destruct (IHrl (if typ_eq t Tsingle
                    then Locset.add (R a) s
                    else Locset.remove (R a) s) tyl) as (s1 & A & B).
    exists s1; split; auto. intros. 
    destruct (t :: tyl); simpl.
    apply A.
  ...
  - destruct (Y lres s (sig_res sg)) as (s' & A & B).
    rewrite A. constructor. red; intros.
    rewrite LSF.diff_iff in H0. destruct H0. apply B. auto. 
    red; intros. exploit list_in_map_inv; eauto. intros (r & U & V).
    subst a. elim H1. rewrite destroyed_at_call_regs_charact. 
...
- (* builtin *)
  ...
  econstructor; eauto.
  apply wt_setregs. 
  admit (*eapply external_call_well_typed'; eauto*).
  auto.
  apply wt_undef_regs; auto.
- (* annot *)
  simpl in *; InvBooleans.
...
- (* external function *)
  ...
  eapply callregs_setregs_result. apply H.
  eapply wt_setregs.
  eapply external_call_well_typed'; eauto.
admit (*  destruct (sig_res (ef_sig ef) ). as [[] | ]; auto.
auto.
(* SIMD *)
  destruct v; auto. auto.*).
auto.
(* eSIMD *)
- (* return *)
