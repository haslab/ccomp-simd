...
Proof.
  ...
  destruct (list_eq_dec Reg.eq r (r1::nil)); try congruence.
  decEq. apply IHn with (r0::nil). auto. omega.
Qed.
...
(*
  ...
  | is_return_move: forall pc r r' s,
      ...
      is_return_spec f s (r'::nil) ->
      (return_measure f.(fn_code) s < return_measure f.(fn_code) pc)%nat ->
     is_return_spec f pc (r::nil).
...
Proof.
  ...
  intros src IMO. destruct (list_eq_dec Reg.eq rret (src::nil)); try congruence.
  subst rret. intro. 
  exploit is_move_operation_correct; eauto. intros [A B]. subst. 
  eapply is_return_move; eauto. eapply IHn; eauto. omega.
  unfold return_measure.
  rewrite <- (is_return_measure_rec f (S n) niter pc (src::nil)); auto.
  rewrite <- (is_return_measure_rec f n niter s (dst::nil)); auto. 
...
Proof.
  ...
  caseEq (is_return niter f n l0 && tailcall_is_possible s &&
          list_typ_eq (sig_res s) (sig_res (fn_sig f))); intros.
...
Lemma regset_setlist:
  forall rs rs' v v' r,
  regset_lessdef rs rs' -> Val.lessdef_list v v' ->
  regset_lessdef (regmap_setlist r v Vundef rs) (regmap_setlist r v' Vundef rs').
Proof.
Admitted.
(*
Qed.
...
Inductive match_states: state -> state -> Prop :=
  ...
  | match_states_return:
      ...
      Val.lessdef v v' ->
      Mem.extends m m' ->
      match_states (Returnstate s (v::nil) m)
                   (Returnstate s' (v'::nil) m')
  | match_states_interm:
...
(* builtin *)
  ...
  left. exists (State s' (transf_function f) (Vptr sp0 Int.zero) pc' (*(rs'#res <- v')*)(regmap_setlist res v' Vundef rs') m'1); split.
  eapply exec_Ibuiltin; eauto.
  eapply external_call_symbols_preserved; eauto.
  exact symbols_preserved. exact varinfo_preserved.
  econstructor; eauto. apply regset_setlist; auto.
...
(* return *)
  exploit Mem.free_parallel_extends; eauto. intros [m'1 [FREE EXT]].
  TransfInstr.
  left. exists (Returnstate s' (rs'##or) m'1); split.
...
(* synchronous return in both programs *)
  left. econstructor; split. 
  apply exec_return. 
  constructor; auto. apply regset_setlist; auto. 
