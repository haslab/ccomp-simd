...
(* compcert-simd: for now, we restrict the Tailcall optimisation on
 single-valued results... *)
Fixpoint is_return (n: nat) (f: function) (pc: node) (rret: list reg) 
                   {struct n}: bool :=
  match n with
  | O => false
  | S n' =>
...
(*      | Some(Ireturn None) => true*)
      | Some(Ireturn nil) => true
      | Some(Ireturn (r::nil)) => list_eq_dec Reg.eq (r::nil) rret
      | Some(Inop s) => is_return n' f s rret
      | Some(Iop op args dst s) =>
          match is_move_operation op args with
          | None => false
          | Some src =>
              if list_eq_dec Reg.eq rret (src::nil)
              then is_return n' f s (dst::nil)
              else false
          end
      | _ => false
      end
...
(*      && opt_typ_eq sig.(sig_res) f.(fn_sig).(sig_res)*)
      && list_typ_eq sig.(sig_res) f.(fn_sig).(sig_res)
