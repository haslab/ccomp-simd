...
Definition kill_list (r: list reg) (ne: nenv) : nenv := 
 fold_left (fun x r=>NE.set r Nothing x) r ne.
...
Function transfer_builtin (app: VA.t) (ef: external_function) (args: list reg) (res: list reg)
                          (ne: NE.t) (nm: nmem) : NA.t :=
  match ef, args with
  | EF_vload chunk, a1::nil =>
      (add_needs_all args (kill_list res ne),
  ...
  | EF_vstore chunk, a1::a2::nil =>
      (add_need_all a1 (add_need a2 (store_argument chunk) (kill_list res ne)), nm)
  | EF_vstore_global chunk id ofs, a1::nil =>
      (add_need a1 (store_argument chunk) (kill_list res ne), nm)
  ...
  | EF_annot txt targs, _ =>
      (add_needs_all args (kill_list res ne), nm)
  | EF_annot_val txt targ, _ =>
      (add_needs_all args (kill_list res ne), nm)
  | _, _ =>
      (add_needs_all args (kill_list res ne), nmem_all) 
  end.
...
Definition transfer (f: function) (approx: PMap.t VA.t)
  ...
  | Some(Icall sig ros args res s) =>
      (add_needs_all args (add_ros_need_all ros (kill_list res ne)), nmem_all)
  ...
  | Some(Ireturn optarg) =>
      (add_needs_all optarg ne, nmem_dead_stack f.(fn_stacksize))
  end.
