...
Definition transfer_call (ae: aenv) (am: amem) (args: list reg) (res: list reg) :=
  if pincl am.(am_nonstack) Nonstack
  && forallb (fun r => vpincl (areg ae r) Nonstack) args
  then
    VA.State (AE.set_all res (Ifptr Nonstack) ae) (mafter_private_call am)
  else
    VA.State (AE.set_all res Vtop ae) mafter_public_call.

Inductive builtin_kind : Type :=
  | Builtin_vload (chunk: memory_chunk) (r:reg) (aaddr: aval)
  | Builtin_vstore (chunk: memory_chunk) (r:reg) (aaddr av: aval)
  | Builtin_memcpy (sz al: Z) (r:reg) (adst asrc: aval)
  | Builtin_annot (r:reg) 
  | Builtin_annot_val (r:reg) (av: aval)
  | Builtin_default.

Definition classify_builtin (ef: external_function) (args: list reg) (res: list reg) (ae: aenv) :=
  match ef, args, res with
  | EF_vload chunk, a1::nil, r::nil => Builtin_vload chunk r (areg ae a1)
  | EF_vload_global chunk id ofs, ni, r::nil => Builtin_vload chunk r (Ptr (Gl id ofs))
  | EF_vstore chunk, a1::a2::nil, r::nil => Builtin_vstore chunk r (areg ae a1) (areg ae a2)
  | EF_vstore_global chunk id ofs, a1::nil, r::nil => Builtin_vstore chunk r (Ptr (Gl id ofs)) (areg ae a1)
  | EF_memcpy sz al, a1::a2::nil, r::nil => Builtin_memcpy sz al r (areg ae a1) (areg ae a2)
  | EF_annot _ _, _, r::nil => Builtin_annot r
  | EF_annot_val _ _, a1::nil, r::nil => Builtin_annot_val r (areg ae a1)
  | _, _, _ => Builtin_default
  end.

Definition transfer_builtin (ae: aenv) (am: amem) (rm: romem) (ef: external_function) (args: list reg) (res: list reg) :=
  match classify_builtin ef args res ae with
  | Builtin_vload chunk r aaddr => 
      let a :=
      if va_strict tt
      then vlub (loadv chunk rm am aaddr) (vnormalize chunk (Ifptr Glob))
      else vnormalize chunk Vtop in
      VA.State (AE.set r a ae) am
  | Builtin_vstore chunk r aaddr av  =>
      let am' := storev chunk am aaddr av in
      VA.State (AE.set r itop ae) (mlub am am')
  | Builtin_memcpy sz al r adst asrc  =>
      let p := loadbytes am rm (aptr_of_aval asrc) in
      let am' := storebytes am (aptr_of_aval adst) sz p in
      VA.State (AE.set r itop ae) am'
  | Builtin_annot r =>
      VA.State (AE.set r itop ae) am
  | Builtin_annot_val r av =>
      VA.State (AE.set r av ae) am
  | Builtin_default =>
...
Lemma classify_builtin_sound:
  forall bc e ae ef (ge: genv) args m t res vres m',
  ematch bc e ae ->
  genv_match bc ge ->
  external_call ef ge e##args m t vres m' ->
  match classify_builtin ef args res ae with
  | Builtin_vload chunk r aaddr =>
      exists addr, res=r::nil /\
      volatile_load_sem chunk ge (addr::nil) m t vres m' /\ vmatch bc addr aaddr
  | Builtin_vstore chunk r aaddr av =>
      exists addr v, res=r::nil /\
      volatile_store_sem chunk ge (addr::v::nil) m t vres m'
      /\ vmatch bc addr aaddr /\ vmatch bc v av
  | Builtin_memcpy sz al r adst asrc =>
      exists dst, exists src, res=r::nil /\
      extcall_memcpy_sem sz al ge (dst::src::nil) m t vres m'
      /\ vmatch bc dst adst /\ vmatch bc src asrc
  | Builtin_annot r => res=r::nil /\
      m' = m /\ vres = nil
  | Builtin_annot_val r av => 
    exists v,  res=r::nil /\ vres = v::nil /\
      m' = m /\ vmatch bc v av
...
- (* vload *)
  ...
  destruct res; auto. destruct res; auto.
  inv H1. exists (e#p); split; eauto.
  split; eauto. rewrite <- H2; constructor; auto.
- (* vstore *)
  destruct args; auto. destruct args; auto. destruct args; auto.
  destruct res; auto. destruct res; auto.
  inv H1.
  exists (e#p), (e#p0); split; try split; eauto.
  rewrite <- H2; constructor; auto.
- (* vload global *)
  destruct args; auto; intros; simpl in H1; try (solve [inv H1]).
  destruct res; auto. destruct res; auto.
  rewrite volatile_load_global_charact in H1. destruct H1 as (b & A & B).
  exists (Vptr b ofs); split; auto. constructor; auto. constructor. 
  constructor. eapply H0; eauto.
- (* vstore global *)
  destruct args; auto. destruct args; auto. intros; simpl in H1.
  destruct res; auto. destruct res; auto. 
  rewrite volatile_store_global_charact in H1. destruct H1 as (b & A & B).
  exists (Vptr b ofs), (e#p); split; auto. split; eauto. split; eauto.
  constructor. constructor. eapply H0; eauto.
- (* memcpy *)
  destruct args; auto. destruct args; auto. destruct args; auto.
  intros; simpl in H1.
  destruct res; auto. destruct res; auto. 
...
Theorem return_from_public_call:
  forall (caller callee: block_classification) bound sp ge e ae vres m rm,
  ...
  (forall v, In v vres -> vmatch callee v Vtop) ->
  romatch callee m rm ->
  mmatch callee m mtop ->
  genv_match callee ge ->
  bc_nostack callee ->
  exists bc,
      (forall v, In v vres -> vmatch bc v Vtop)
   /\ ematch bc e ae
   /\ romatch bc m rm
...
(* Invariance properties *)
  ...
  {
    intros. assert (vmatch callee v Vtop) by (eapply vmatch_top; eauto). 
...
Theorem return_from_private_call:
  forall (caller callee: block_classification) bound sp ge e ae vres m rm am,
  bc_below caller bound ->
  callee sp = BCinvalid ->
  caller sp = BCstack ->
  (forall b, Plt b bound -> b <> sp -> caller b = callee b) ->
  ...
  bc_nostack callee ->
  exists bc,
      (forall v, In v vres -> vmatch bc v (Ifptr Nonstack))
   /\ ematch bc e ae
   /\ romatch bc m rm
...
(* é preciso generalizar (vres::nil) para (vres)!!!! *)
...
Inductive sound_stack: block_classification -> list stackframe -> mem -> block -> Prop :=
  ...
  | sound_stack_public_call:
      forall (bc: block_classification) res f sp pc e stk m bound bc' bound' ae
        ...
        (GE: genv_match bc' ge)
        (AN: VA.ge (analyze rm f)!!pc (VA.State (AE.set_all res Vtop ae) mafter_public_call))
        (EM: ematch bc' e ae),
  ...
  | sound_stack_private_call:
     forall (bc: block_classification) res f sp pc e stk m bound bc' bound' ae am
        ...
        (SAME: forall b, Plt b bound' -> b <> sp -> bc b = bc' b)
        (GE: genv_match bc' ge)
        (AN: VA.ge (analyze rm f)!!pc (VA.State (AE.set_all res (Ifptr Nonstack) ae) (mafter_private_call am)))
        (EM: ematch bc' e ae)
...
Inductive sound_state: state -> Prop :=
  ...
  | sound_return_state:
      forall s v m bc
        (STK: sound_stack bc s m (Mem.nextblock m))
        (RES: forall x, In x v -> vmatch bc x Vtop)
        (RO: romatch bc m rm)
        (MM: mmatch bc m mtop)
        (GE: genv_match bc ge)
        (NOSTK: bc_nostack bc),
...
Lemma ematch_updatelist:
  forall bc r e ae vres av,
  ematch bc e ae -> 
  vmatch bc Vundef av ->
  (forall v, In v vres -> vmatch bc v av) ->
  ematch bc (regmap_setlist r vres Vundef e) (AE.set_all r av ae).
Proof.
  induction r; simpl; intros; auto.
  destruct vres; simpl.
   intros; apply ematch_update; auto.
  apply ematch_update; auto.
   apply IHr; auto.
   intros; apply H1; simpl; right; auto.
  apply H1; simpl; left; auto.
Qed.
...
- (* builtin *)
  ...
  exploit (classify_builtin_sound bc rs ae ef ge args m t res) ; eauto.
  
  destruct (classify_builtin ef args res ae). 
+ (* volatile load *)
  intros (addr & RES & VLOAD & VADDR). subst; inv VLOAD.
  eapply sound_succ_state; eauto. simpl; auto.
  simpl. apply ematch_update; auto. 
  inv H2.
...
+ (* volatile store *)
  intros (addr & src & RES & VSTORE & VADDR & VSRC). subst; inv VSTORE. inv H7.
  * (* true volatile access *)
    eapply sound_succ_state; eauto. simpl; auto.
    simpl; apply ematch_update; auto. constructor.
...
+ (* memcpy *)
  intros (dst & src & RES & MEMCPY & VDST & VSRC). subst; inv MEMCPY.
  eapply sound_succ_state; eauto. simpl; auto. 
  apply ematch_update; auto. constructor.
...
+ (* annot *)
  intros (RES & A & B); subst. 
  eapply sound_succ_state; eauto. simpl; auto. 
  apply ematch_update; auto. constructor.
+ (* annot val *)
  intros (v' & RES & VRES & A & B); subst. 
  eapply sound_succ_state; eauto. simpl; auto. 
  apply ematch_update; auto.
+ (* general case *)
  intros _.
...
* (* private builtin call *)
  ...
  apply ematch_updatelist; auto. constructor.
  apply sound_stack_exten with bc. 
...
* (* public builtin call *)
  ...
  intros (bc3 & U & V & W & X & Y & Z & AA).
  eapply sound_succ_state with (bc := bc3); eauto. simpl; auto.
  apply ematch_updatelist; auto.
  apply vmatch_top with (Uns 0). constructor.
  apply sound_stack_exten with bc. 
  apply sound_stack_inv with m. auto.
  intros. apply Q. red. eapply Plt_trans; eauto.
  rewrite C; auto.
...
- (* return *)
  ...
  elim or; intros; simpl; auto. inv H1.
...
- (* return *)
  ...
  + (* from private call *)
   ...
   apply sound_stack_exten with bc'; auto.
   eapply ematch_ge; eauto.
   apply ematch_updatelist; eauto. constructor.
