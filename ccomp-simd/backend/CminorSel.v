...
Inductive stmt : Type :=
  ...
  | Scall : list ident -> signature -> expr + ident -> exprlist -> stmt
  | Stailcall: signature -> expr + ident -> exprlist -> stmt
  | Sbuiltin : list ident -> external_function -> exprlist -> stmt
  | Sseq: stmt -> stmt -> stmt
...
(** ccomp-simd: builtins and external functions appearing in [expr] are
  retricted to single-valued *)
  | eval_Ebuiltin: forall le ef al vl v,
      eval_exprlist le al vl ->
      external_call ef ge vl m E0 (v::nil) m ->
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | step_builtin: forall f optid ef al k sp e m vl t v m',
      ...
      step (State f (Sbuiltin optid ef al) k sp e m)
         t (State f Sskip k sp (set_ret optid v e) m')
...
*)
  | step_return_1: forall f a k sp e m v m',
      eval_exprlist (Vptr sp Int.zero) e m nil a v ->
      Mem.free m sp 0 f.(fn_stackspace) = Some m' ->
