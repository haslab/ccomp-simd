...
Definition transfer
  ...
  | Some i =>
      ...
      | Icall sig ros args res s =>
          reg_list_live args
           (reg_sum_live ros (reg_list_dead res after))
      | Itailcall sig ros args =>
          reg_list_live args (reg_sum_live ros Regset.empty)
      | Ibuiltin ef args res s =>
          reg_list_live args (reg_list_dead res after)
      | Icond cond args ifso ifnot =>
          reg_list_live args after
      | Ijumptable arg tbl =>
          reg_live arg after
      | Ireturn optarg =>
          reg_list_live optarg Regset.empty
      end
  end.
...
Definition last_uses_at (live: PMap.t Regset.t) (pc: node) (i: instruction) : list reg :=
  ...
  fold_right (fun r X=>if Regset.mem r l then X else r :: X) lu (instr_defs i).
