...
let botret (x: 'a) : 'a botlift = NotBot x
...
let string_of_botlift f =
  function
  | Bot -> "⊥"
  | NotBot a -> Camlcoq.camlstring_of_coqstring (f a)

type ('ab, 'instr) adom = {
  order: 'ab botlift -> 'ab botlift -> bool;
  join: 'ab botlift -> 'ab botlift -> 'ab botlift;
  widen: 'ab botlift -> 'ab botlift -> 'ab botlift;
  init: unit -> 'ab botlift;
  transfer: int -> 'instr -> 'ab -> 'ab botlift;
  to_string: 'ab -> string
}
...
let check_fixpoint adom cfg res =
  if not (adom.order (adom.init ()) res.(cfg.entry)) then
    failwith "res.(ENTRY) should be overapproximate the initial state !\n";
  Array.iteri
      (fun i succs ->
         List.iter
         (fun (j,op) ->
            if not (match res.(i) with NotBot i' -> adom.order (adom.transfer i op i') res.(j) | Bot -> true) then
              begin
...
let run_with_bourdoncle adom b cfg =
  ...
  let res = Array.init (Array.length preds) (fun _ -> Bot) in
  let join_list = f_list adom.join in
  let _ = res.(cfg.entry) <- adom.init () in
    let upd j =
      if j <> cfg.entry then
        res.(j) <-
          join_list Bot
          (List.map (fun (i,op) -> botbind (adom.transfer i op) res.(i)) preds.(j)) in
    let upd_except_cfc j cfc =
      if j <> cfg.entry then
        res.(j) <- join_list res.(j)
          (List.map (fun (i,op) -> botbind (adom.transfer i op) res.(i))
