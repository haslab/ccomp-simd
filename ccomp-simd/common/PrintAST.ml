...
let rec string_of_intlit_list = function
  | [] -> ""
  | (x::[]) -> sprintf "%Ld" x
  | (x::xs) -> sprintf "%Ld, %s" x (string_of_intlit_list xs)

let name_of_external = function
  | EF_external(name, sg) -> sprintf "extern %S" (extern_atom name)
  | EF_builtin(name, Some data, sg) -> 
     if data.blt_imms = 0
     then sprintf "simd %S" (extern_atom name)
     else sprintf "simd %S with %s" (extern_atom name)
		  (string_of_intlit_list (data.blt_immargs))
  | EF_builtin(name, None, sg) -> 
     sprintf "builtin %S" (extern_atom name)
