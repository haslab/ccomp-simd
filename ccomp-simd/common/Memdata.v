...
Definition size_chunk (chunk: memory_chunk) : Z :=
  ...
  | Many32 => 4
  | Many64 => 8
  | Mvec t => typesize (type_of_chunk chunk)
  end.
...
Proof.
  ...
  unfold size_chunk_nat, size_chunk, typesize, typelocsize. 
  rewrite Z2Nat.id; auto.
  generalize (two_power_nat_pos (typerank (type_of_chunk (Mvec v)))); omega.
Qed.
...
Proof.
  intros. destruct chunk1; destruct chunk2; 
  simpl in *;
  try solve [ omegaContradiction
        | apply Z.divide_refl
        | exists 2; reflexivity
        | exists 4; reflexivity
        | exists 8; reflexivity ];
solve [ apply (Z.divide_trans _ 8);
            [| apply vec_align_8divides];
          solve [ apply Z.divide_refl
                | exists 2; reflexivity
                | exists 4; reflexivity
                | exists 8; reflexivity ]
      | destruct v; unfold vec_align, typesize,
        typelocsize in H; simpl in *;
        omegaContradiction
      | apply vec_align_le_divides; auto].
Qed.
...
Proof.
  ...
  decEq. apply Z.mod_small. apply Int128.unsigned_range. apply Int128.repr_unsigned.
Qed.

Lemma decode_encode_int_32:
  forall x, Int256.repr (decode_int (encode_int 32 (Int256.unsigned x))) = x.
Proof.
  intros. rewrite decode_encode_int. transitivity (Int256.repr (Int256.unsigned x)).
  decEq. apply Z.mod_small. apply Int256.unsigned_range. apply Int256.repr_unsigned.
Qed.

Lemma decode_encode_int_64:
  forall x, Int512.repr (decode_int (encode_int 64 (Int512.unsigned x))) = x.
Proof.
  intros. rewrite decode_encode_int. transitivity (Int512.repr (Int512.unsigned x)).
  decEq. apply Z.mod_small. apply Int512.unsigned_range. apply Int512.repr_unsigned.
Qed.
...
Definition encode_val (chunk: memory_chunk) (v: val) : list memval :=
  ...
  | (Vint _ | Vsingle _ | Vlong _ | Vfloat _ | Vptr _ _), Many32 => inj_value Q32 v
  | (Vint _ | Vsingle _ | Vlong _ | Vfloat _ | Vptr _ _), Many64 => inj_value Q64 v
  | Vvec t v, Mvec t' => if Archi.vec_typ_eq t t'
...
Ltac solve_encode_val_length :=
  ...
  | [ |- length (list_repeat ?x _) = ?x ] => rewrite length_list_repeat; reflexivity
  | _ => reflexivity
  end.
...
Definition decode_encode_val (v1: val) (chunk1 chunk2: memory_chunk) (v2: val) : Prop :=
  ...
  | Vint n, (Mint64 | Mfloat32 | Mfloat64 | Many64 | Mvec _), _ => v2 = Vundef
  ...
  | Vlong n, (Mint8signed|Mint8unsigned|Mint16signed|Mint16unsigned|Mint32|Mfloat32|Mfloat64|Many32|Mvec _), _ => v2 = Vundef
  ...
  | Vfloat f, (Mint8signed|Mint8unsigned|Mint16signed|Mint16unsigned|Mint32|Mfloat32|Mint64|Many32|Mvec _), _ => v2 = Vundef
  ...
  | Vsingle f, (Mint8signed|Mint8unsigned|Mint16signed|Mint16unsigned|Mint32|Mint64|Mfloat64|Many64|Mvec _), _ => v2 = Vundef
  | Vsingle f, _, _ => True (* nothing interesting to say about v2 *)
  | Vvec t v, Mvec t1, Mvec t2 => if Archi.vec_typ_eq t t1 
                                  then if Archi.vec_typ_eq t t2
                                       then v2 = Vvec t v
                                       else True
                                  else v2 = Vundef
  | Vvec t v, Mvec t1, _ => if Archi.vec_typ_eq t t1 
                            then True
                            else v2 = Vundef
...
Lemma proj_bytes_repeat_undef chunk:
 proj_bytes (list_repeat (size_chunk_nat chunk) Undef) = None.
Proof.
generalize (size_chunk_nat_pos chunk).
induction (size_chunk_nat chunk); intros [n' Hn'].
 discriminate Hn'.
auto.
Qed.

Lemma proj_value_repeat_undef q chunk:
 proj_value q (list_repeat (size_chunk_nat chunk) Undef) = Vundef.
Proof.
generalize (size_chunk_nat_pos chunk).
induction (size_chunk_nat chunk); intros [n' Hn'].
 discriminate Hn'.
...
Lemma vec_decode_encode_val t v:
 Vvec t (vec_repr (decode_int (encode_int (size_chunk_nat (Mvec t)) (vec_intval v)))) = Vvec t v.
Proof.
destruct t; simpl; 
rewrite ?decode_encode_int_16, ?decode_encode_int_32, ?decode_encode_int_64; auto.
Qed.
...
Ltac solve_decode_encode_val_general :=
  ...
  | |- context [ Int128.repr(decode_int (encode_int 16 (Int128.unsigned _))) ] => rewrite decode_encode_int_16
  | |- context [ Int256.repr(decode_int (encode_int 32 (Int256.unsigned _))) ] => rewrite decode_encode_int_32
  | |- context [ Int512.repr(decode_int (encode_int 64 (Int512.unsigned _))) ] => rewrite decode_encode_int_64
  ...
  | |- (match ?c return _ with | left _ => _ | right _ => _ end) => case c; intros
  | |- context [ proj_bytes (list_repeat _ Undef) ] => rewrite proj_bytes_repeat_undef
  | |- context [ proj_value _ (list_repeat _ Undef) ] => rewrite proj_value_repeat_undef
  end.
...
- subst; rewrite vec_decode_encode_val; reflexivity.
Qed.
...
Proof.
  ...
  destruct v1; auto.
injection TY as ->.
revert DE; unfold decode_encode_val, Val.load_result;
case (Archi.vec_typ_eq t v0); auto.
Qed.
...
Definition quantity_chunk (chunk: memory_chunk) :=
  match chunk with
  | Mint64 | Mfloat64 | Many64 => Some Q64
  | _ => Some Q32
  end.
...
Inductive shape_encoding (chunk: memory_chunk) (v: val): list memval -> Prop :=
  ...
  | shape_encoding_b: forall b mvl,
      match v with Vint _ => True | Vlong _ => True | Vfloat _ => True | Vsingle _ => True | Vvec _ _ => True | _ => False end ->
...
Local Transparent inj_value.
  ...
  (apply B || apply C || apply D || (destruct Archi.ptr64; (apply B || apply D || idtac)));
  auto;
revert LEN; case (Archi.vec_typ_eq t v0); intros;
(apply C || apply D); auto.
Qed.
...
- destruct Archi.ptr64; auto using inj_value_inject, repeat_Undef_inject_self.
- case (Archi.vec_typ_eq t v0); intros _.
