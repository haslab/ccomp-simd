...
let next_atom = ref (Coq_xO Coq_xH) (*obs: atom 1 is used for debug purposes*)

let set_debug_string s = Hashtbl.add string_of_atom Coq_xH s
let with_debug_string f = 
  try f (Hashtbl.find string_of_atom Coq_xH)
  with Not_found -> ()
...
let rec intern_strings = function
  | [] -> []
  | x::xs -> let i = intern_string x 
             and il = intern_strings xs in i::il
...
let coqfloat_of_camlint64 i =
  Float.double_of_bits(coqint_of_camlint64(i))
let camlint64_of_coqfloat f =
  camlint64_of_coqint(Float.bits_of_double f)
