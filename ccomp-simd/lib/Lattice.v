...
Definition set_all (p: list positive) (v: L.t) (x: t) : t :=
  fold_right (fun r y => set r v y) x p.

Lemma set_all_nbot: forall l v x,
  x <> Bot -> ~L.eq v L.bot -> set_all l v x <> Bot.
Proof.
induction l; simpl; intros; auto.
assert (Hx: set_all l v x <> Bot); auto.  
unfold set.
destruct (set_all l v x); auto.
case_eq (L.beq v L.bot).
 intro HH. apply L.beq_correct in HH. elim H0; auto.
...
Lemma gsspec_all:
  forall p v x q,
  x <> Bot -> ~L.eq v L.bot ->
  L.eq (get q (set_all p v x)) (if in_dec peq q p then v else get q x).
Proof.
  induction p; intros; auto.
  - apply L.eq_refl.
  - simpl set_all.
    apply L.eq_trans with (if peq q a then v else get q (set_all p v x)).
    apply gsspec; auto. apply set_all_nbot; auto.
    case (peq q a); intro Ha.

    subst; simpl. destruct (peq a a). apply L.eq_refl. elim n; reflexivity.

    cutrewrite ((if in_dec peq q (a :: p) then v else get q x) = (if in_dec peq q p then v else get q x)).

    apply IHp; auto.

    simpl. destruct (peq a q). rewrite e in Ha; elim Ha; reflexivity.
    destruct (in_dec peq q p); auto.
Qed.
