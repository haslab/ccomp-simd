...
Lemma divides_align_eq: forall x y,
  0 < x -> y >= 0 -> (x|y) -> y = align y x.
Proof.
intros.
destruct (Z.eq_dec y 0).
 subst; unfold align.
 rewrite Z.add_0_l. cutrewrite ((x - 1) / x = 0); [omega|].
 apply Zdiv_small. omega.
destruct H1 as [z Hz]; rewrite Hz.
unfold align.
cutrewrite (z * x + x - 1 = z * x + (x - 1)); [|ring].
rewrite Z_div_plus_full_l;[|omega].
ring_simplify.
cutrewrite ((x - 1) / x = 0);[omega|].
apply Zdiv_small; omega.
Qed.
